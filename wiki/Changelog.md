...
- **Windows README Startup Is Less Likely To Fall Into a Broken Conda Path**: The PowerShell launcher now checks more standard Windows Python install locations before relying on the Pinokio/conda bootstrap path, a failed conda bootstrap no longer hard-aborts local installs that should fall back to a normal Python setup, and the launcher/docs now match the app’s actual Python 3.10+ compatibility.
