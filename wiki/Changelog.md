...
- **Windows README Startup Is Less Likely To Fall Into a Broken Conda Path**: The PowerShell launcher now checks more standard Windows Python install locations before relying on the Pinokio/conda bootstrap path, and a failed conda bootstrap no longer hard-aborts local installs that should fall back to a normal Python 3.11 setup.
