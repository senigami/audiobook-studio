...
The real plugins/tts_xtts/plugin/assets/ folder is a live, shared location (populated by actual
...
<root>/plugin/server/engine.py layout, mirroring the real plugins/tts_xtts/plugin/server/engine.py
directory depth.
"""
...
def _make_isolated_plugin_dir(tmp_path: Path) -> Path:
    ...
    tmp_path plugin root — mirroring the real plugins/tts_xtts/plugin/server/engine.py
