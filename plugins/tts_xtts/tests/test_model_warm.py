...
def test_model_warm_returns_false_when_manager_not_initialized():
    ...
    with patch("plugins.tts_xtts.plugin.core.implementation._warm_worker_manager", None):
...
def test_model_warm_returns_true_when_manager_is_ready():
    ...
    with patch("plugins.tts_xtts.plugin.core.implementation._warm_worker_manager", mock_mgr):
        from plugins.tts_xtts.plugin.server.engine import XttsPlugin
        plugin = XttsPlugin()
