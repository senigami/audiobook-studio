"""Tests for plugins/tts_xtts/plugin/core/diagnostics.py's stderr write lock.
...
from plugins.tts_xtts.plugin.core.diagnostics import emit_diagnostics, emit_stderr_atomic


def test_emit_diagnostics_calls_on_output_and_tees_to_stderr(monkeypatch):
    written: list[str] = []
    monkeypatch.setattr(
        "plugins.tts_xtts.plugin.core.diagnostics.sys",
...
def test_engine_module_shares_the_same_lock_instance():
    ...
    from plugins.tts_xtts.plugin.core import diagnostics as diag_module
