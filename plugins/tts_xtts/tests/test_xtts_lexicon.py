...
from plugins.tts_xtts.plugin.studio import handler as _xtts_handler  # noqa: F401
from plugins.tts_xtts.plugin.studio import bake as _bake_mod
from plugins.tts_xtts.plugin.studio import segments as _seg_mod
from plugins.tts_xtts.plugin.studio import standard_handler as _std_mod
...
class TestXttsBakeLexicon:
    ...
    def _run(self, project_id, lexicon_entries, text="SQL is fast."):
        ...
        with patch.object(bk, "_get_ctx", return_value=mock_ctx), \
             ...
             patch("plugins.tts_xtts.plugin.studio.helpers._profile_inputs_for_segment",
...
class TestXttsSegmentsLexicon:
    ...
    def _run(self, project_id, lexicon_entries, text="SQL is fast."):
        ...
        with patch.object(seg, "_get_ctx", return_value=mock_ctx), \
             ...
             patch("plugins.tts_xtts.plugin.studio.helpers._profile_inputs_for_segment",
                   return_value=("ref.wav", None)), \
             patch("plugins.tts_xtts.plugin.studio._text_utils.join_group_text",
                   side_effect=lambda grp: " ".join(s["text_content"] for s in grp)), \
             patch("plugins.tts_xtts.plugin.studio._text_utils.build_segment_groups",
