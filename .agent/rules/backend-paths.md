...
- For profile metadata reads and repairs, scan the parent directory and match the exact metadata filename by `entry.name` before opening it; do not rely on `os.path.exists()` on a derived metadata path when CodeQL is tracing the source.
- If CodeQL still traces a metadata or discovery scan, prefer a direct `try/except os.scandir(validated_parent)` block over a separate `os.path.isdir()` guard on the same derived path, so the root proof and sink stay in the same control-flow block.
