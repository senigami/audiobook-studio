...
- If `design-docs/code-map/map.json` exists and this task added, removed, or changed the contract/exports/calls of a mapped file (see `meta.unmapped` in the map for what's excluded), append ONE small JSON entry to `design-docs/code-map/queue/` before declaring the task done — same-change rule, not a follow-up. Name it `<UTC-timestamp>-<task-slug>.json`; never edit `map.json` directly or touch another entry. Shape: `{"at", "commit", "task", "files": {"<path>": {"change": "added|modified|removed", "contract_changed": bool, "summary": "...", "calls_added": [...], "calls_removed": [...]}}, "flow_impact": "one line or 'none'"}`. This is cheap (a few hundred tokens from whichever agent already holds the diff) and is what keeps the map from silently drifting — a 2026-07-03 session found the map two full commits stale in part because this step was skipped.
