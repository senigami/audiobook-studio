...
- **Constance: substantially build-ready, one correctness MUST-FIX** (N1: fragment runs aren't
...
- **Petra: NOT build-ready — one BLOCKER.** Task 0's stated behavior (preserve content found at a
  different index) necessarily preserves the "Middle" sentence across its one-position move in the
  existing frozen test — but that test's current assertion is `Middle → unprocessed`. Petra traced
...
Constance and Petra also converged independently on the same underlying issue from different
angles: Constance's N2 (preserve vs. re-index is contradictory) and Petra's blocker (the frozen
...
1. **State explicitly (00-overview.md / Task 0) that the frozen test's *intent* — reordered
   ...
   preserved, not `unprocessed`, once Task 0 lands). This is Petra's blocker, and it's a plan-text
   fix, not a design change.
2. **Remove the 1-3 fragment-run cap; make matching unbounded/prefix-driven** (Constance's N1) —
...
3. **Reconcile "preserve in place" with position re-indexing** (Constance's N2) — preserve means
...
4. **Correct Invariant I3's cited code path** (Petra's finding) — the DB path uses
...
5. **Name one concrete alignment algorithm** (Petra) — not just "content-match, position
