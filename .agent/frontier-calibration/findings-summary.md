...
| **PL-2** | Standalone-plugin extraction plan (8 slices, "never touch the bundled in-tree copy until the final clean-machine test"). Flags that **much of doc 05 is already shipped** (SDK inversion, distribution blocks, tts_mixed rename/registration, backend install/trust E2E) — the genuinely open work is repo creation + §5.3 UI acceptance + §5.1 clean-machine E2E + Group 6 docs. | Realizes the plugin-marketplace promise; reconciles an overclaiming/underclaiming plan doc. | Doc 010 / doc 05 reconciliation (an Astrid/archivist task) + the execution plan. | Plan doc — draftable; reconcile doc 05 after PL-2 twin run |
