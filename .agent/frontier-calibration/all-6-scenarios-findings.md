...
(Phase 2), and independent Fable + Esther + Tamsin plan reviews (Phase 3) — RC-1 additionally
...
- **Esther resolved the plan's one blocking open question**: Studio can't read CUDA in-process
...
- **Tamsin found a real arithmetic bug**: the design subtracted the penalty *before* the
...
- **Tamsin also disagreed with Esther on transport** — NVML/`nvidia-smi` from the Studio daemon
  (reads global board memory, correct even if synthesis moves remote) vs. Esther's `/health`
...
- **Tamsin flagged a goal-honesty gap**: the mechanism can only stop *new* admissions, not evict a
...
- **Tamsin flagged an ETA-jitter regression risk**: the same chokepoint feeds the ETA bracket, so a
...
- **Both Esther and Tamsin caught the same critical defect**: the plan's cited shim precedent
...
- **Esther flagged an architecture trade the plan defaulted past**: the recommended destination
...
- **Tamsin found the Stage-5 gate is unsatisfiable as written**: repo-wide `grep "app\.jobs"`
  ...
  the scoping.
- Minor: Tamsin also caught an off-by-one (7 vs. actual 8 `tts_engines` import sites).
...
on the fix, with Esther's converging as strictly better:**
...
- **Tamsin's proposed fix**: an opaque admission-path token stored on the (mutable) task object,
  threaded through every release site.
- **Esther's proposed fix — simpler, adopted**: since every gate's `release()` is already
...
- **Esther found the F5 fix mechanism was infeasible as specified**: normalization always
...
- **Both Esther and Tamsin caught the same material omission**: doc 05 §3's "KEEP `plugins/`"
  decision is now completely inverted by reality (the `tts_engines/` rename already shipped) — a
  bigger, unaddressed drift than the two small items the addendum did flag. Tamsin additionally found
  broken find-replace artifacts in doc 05's prose ("GitHub changed from GitHub to GitHub").
- **Esther found the sync-guard item (new Slice 4) is under-specified**: the registry has no
  `version` field at all — only the URL half is currently checkable.
- **Tamsin found the in-tree version already drifted from doc 05's stated version** (`1.0.1` vs.
