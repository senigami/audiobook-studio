...
- **Stage Direction** (skip marker): marks a span as *excluded from the TTS queue entirely*. The text remains in the document and is visible on screen — it is never rendered as audio and never passed to the TTS engine. Keyboard shortcut `S` in Cast mode. Appears as a built-in system entry in the Cast palette (like Narrator — always present, not user-created). **Visual treatment:** Geist Mono, slightly smaller than prose, muted gray — no character tint. Reads like a printed stage direction (Courier aesthetic, typographically subordinate). A ⊘ glyph appears in the annotation gutter for each skipped span so they're identifiable without reading the text.
  ...
  **Stage Direction also serves as the inline note and emotion-intent system.** Rather than a separate notes field or annotation drawer, a director or author can write emotional intent directly into the text flow and paint it Stage Direction. The narrator sees it exactly where it applies in the reading column — right before or after the line it governs — and the TTS engine skips it entirely:

  > *She picked up the letter.*
  > `[Elena is hiding something — voice catches, trailing off]`  ← painted Stage Direction
  > *"I've never seen this before."*

  This resolves both the playwright's parenthetical-cue problem (`[bitterly]` painted Stage Direction rather than spoken or stripped) and the narrator performer's need for inline performance notes without navigating to a separate panel. The note is spatially where the performer needs it.

  Applies to: stage directions, action lines, parenthetical tone cues (`[bitterly]`, `(quietly)`), chapter headings embedded in the manuscript, epigraphs, section dividers (`***`), author's notes, inline performance/emotion direction — any text that belongs on the page but not in the audio. **Auto-detect on import:** the import annotation extractor (§16) automatically paints Stage Direction on Fountain action lines and parentheticals, queuing them for confirmation.
...
- **Karaoke highlight + scroll-follow:** the playing span lights up, surrounding text dims, the line auto-centers. Feasible via the **CSS Custom Highlight API** (no DOM surgery — the spans already exist). Reduced-motion path = instant jump, no smooth scroll, no per-character animation. **Stage Direction spans are skipped by the karaoke highlight and the playhead** — the visual cursor jumps over them as if they don't exist in the audio timeline; they remain on screen in their Geist Mono treatment.
- **Tap any line to play from there** (like clicking a DAW waveform). This is the primary verb — every line looks tappable. Tapping a Stage Direction span seeks to the next renderable segment after it.
...
| **Stage Direction (skip marker)** | Built-in system assignment in the Cast palette (always present, not user-created). Shortcut `S` in Cast mode. Marks a span as **excluded from the TTS queue entirely** — text stays visible, audio is never generated for it. Data model: `speaker_id = "_stage_direction"` (sentinel) + `render: false` on the segment; render queue filters these out. Visual: Geist Mono, muted gray, ⊘ glyph in annotation gutter. **Dual role:** (1) structural non-speech content (stage directions, action lines, headings, epigraphs, dividers), and (2) **inline performance and emotion intent** — directors and authors write emotional/performance direction directly in the text flow and paint it Stage Direction; the narrator reads it where it appears; the TTS skips it. This replaces the need for a separate "director's notes" drawer. Auto-paintable by the import annotation extractor on Fountain import (§16). **Ships in v1.** |
