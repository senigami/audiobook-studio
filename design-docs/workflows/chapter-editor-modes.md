...
- **Performance Cue** (engine directive): the companion to Stage Direction. Displays in the same Geist Mono / muted visual style inline in the document, but carries a structured SSML payload that *is* sent to the TTS engine for the segment that follows it. Created via the **Cue Editor** — a small inline popover (not a mode, not a drawer) with two fields:
  1. **SSML parameter pickers** — Rate (slow / normal / fast), Pitch (low / normal / high), Volume (soft / normal / loud), and an optional free-text style prompt for models that accept natural language direction.
  2. **Description** (optional) — free-form human note; shown in the inline display but never sent to the engine.

  The Cue Editor generates a combined human-readable inline display by translating the selected SSML values to natural-language labels and appending the description:

  > `[slowly · low | voice catches, trailing off]`

  Left of `|` = structured engine values (translated). Right of `|` = human description. The engine receives the SSML payload; the human sees the full combined annotation.
...
| **Performance Cue (engine directive)** | Built-in system assignment in Cast palette — companion to Stage Direction. Carries a structured SSML payload sent to the TTS engine for the segment that follows it. Created via the **Cue Editor** inline popover: (1) SSML pickers — Rate (slow/normal/fast), Pitch (low/normal/high), Volume (soft/normal/loud), optional style prompt text field; (2) optional free-form description for human readers. Displays inline in the same Geist Mono / muted style as Stage Direction, with a combined human-readable label: `[slowly · low | voice catches, trailing off]` (left of `|` = translated SSML values; right = description). **⚡ glyph** in annotation gutter distinguishes from ⊘. Engine payload ignored silently on engines that don't support SSML (XTTS); consumed automatically when a capable engine is used. **Data model and display ship v1; engine consumption is engine-dependent.** |
