...
| `start_offset` | INTEGER | #232 Task 005: byte offset into `chapters.text_content` where this row's text begins. Populated for every row by migration version 2 (`ux_seg_start` unique per `chapter_id`); NULL only on a schema between migration 1 and 2. |
| `end_offset` | INTEGER | #232 Task 005: byte offset where this row's text ends (exclusive). Same population/uniqueness story as `start_offset` (`ux_seg_end`). No DB-level `end_offset > start_offset` CHECK yet — see the 1.15.0 changelog row for why. |
| `text_hash` | TEXT | #232 Task 001/005: `segment_text_hash(text_content)` (`sha256(text_content.strip())`), the write-back fingerprint guard's (Task 003) comparison key. Never NULL from migration 1 onward. |

#### Segment audio artifacts & orphan reconciliation

**Pre-#232-Task-005 / not-yet-resynced-post-collapse rows:** rendering groups consecutive compatible segments into a **render group** and synthesizes **one** WAV per group, written to `projects/<project_id>/chapters/<chapter_id>/segments/` and named `<batch_timestamp_ns>_<leader_segment_order_index>.wav`. On success that single filename is written as the `audio_file_path` of **every** member segment of the group (the **group→filename fan-out**): many `chapter_segments` rows share one bare filename.

**Post-Task-005 collapsed rows:** one row = one file, enforced by `ux_seg_audio_file` (see the 1.15.0 changelog row). A chapter's rows are in this state immediately after the collapse migration runs; they revert to the fan-out shape above the moment `sync_chapter_segments` creates fresh sentence-grain rows for edited text, since that function is not yet wired to render-block grouping (Task 005b) — the fan-out write for those fresh rows will violate `ux_seg_audio_file` until 005b ships. Do not treat this section as internally consistent for a chapter mid-transition between the two shapes; it describes the two endpoints, not a merged model.
