...
| 1.13.3  | 2026-08-26 | **Third finding in the same feature: the 1.13.2 fix only closed the LIVE broadcast half of the render-batch-count bug.** `ChapterSynthesisTask._publish_progress` has two separate writes per tick — `service.publish(...)` (the WS broadcast, fixed in 1.13.2) and a "Durable refresh (2026-07-07 fix)" `update_job(..., **update_kwargs)` call that keeps the job ROW current for REST-hydrated reads. `update_kwargs` never included `render_group_count`/`completed_render_groups`, so a restarted app's job row showed both as `null` even after real progress ticks (confirmed live in `state.json`). Effect: any REST-hydrated read (page load, reconnect, a fresh queue fetch, recovery-after-restart) still fell back to the raw per-sentence segment count, self-healing only once a WS tick happened to land on an already-open tab. Fixed: `update_kwargs` now unconditionally includes `render_group_count`/`completed_render_groups` (always available at this point, unlike the optional `eta_seconds`). Regression test: `tests/orchestration/test_chapter_render_group_counts_durable.py` (R1 revert-checked — red on pre-fix code, green after). |
...
| 1.10.0  | 2026-07-03 | **W-PAR task 007 — cap-default-1 toggle surfaced as a Studio setting (§7.3b) + per-engine-id admission ceiling (§7.4a, folded-in merge-gate finding).** `tts_parallel_cap` (global, default 1) and `tts_engine_caps` (per-engine override dict) are now real settings (`GET/POST /api/settings`), falling back to `TTS_PARALLEL_CAP`/`TTS_ENGINE_CAPS` env vars — same settings-then-env precedence as `api_priority_mode`. `app.orchestration.scheduler.cap_settings.resolve_effective_cap(engine_id, manifest_max)` computes `min(requested_cap, manifest_max)` with no engine-ID branching (INV-5); the manifest is always the ceiling. Default stays byte-identical to pre-007 (`tts_parallel_cap=1`). Also closes a latent gap in the grow-only class semaphore (commit `7dd218aa`): a new independent per-`engine_id` semaphore registry (`get_engine_id_semaphore`) is checked alongside the class-level gate whenever a claim declares `engine_id`, so one engine's declared cap can never be inflated by a same-class sibling's larger request. Not observable today (only XTTS resolves to `"gpu"`; additive/opt-in for callers that don't declare `engine_id`). `EngineClassSemaphore` also now hard-rejects growing the `"exclusive"` class above cap=1. |
...
(`tts_engines/tts_xtts/plugin/studio/standard_handler.py`), xtts bake
(`tts_engines/tts_xtts/plugin/studio/bake.py`), and voxtral bake
(`tts_engines/tts_voxtral/plugin/studio/bake.py`) — so rebuild behaves identically
...
`render_one_group` (extracted from `tts_engines/tts_mixed/handler.py`'s
...
wire `tts_engines/tts_mixed/handler.py`'s `_group_needs_render`/
...
### 7.4a Per-engine-id admission ceiling — W-PAR task 007 (folded-in review finding)
...
cancel poll in the xtts worker loop (`tts_engines/tts_xtts/plugin/core/xtts_inference.py`)
