...
- The `LOADING_MODEL` / `running+indeterminate` frame is emitted by `orchestrator_helpers.py` via `ProgressService.publish`. The proactive `pre_load_eta` / `preparing` frame is emitted just before dispatch begins (~line 1336).
