...
| Version | Date       | Change                  |
|---------|------------|-------------------------|
| 1.12.0  | 2026-08-27 | **§7A render monitor is now batch-granular, never sentence-granular (owner ruling, `glossary.md` 1.1.0 — "segment batches are as fine grained as this gets").** `useSegmentInventory.ts` (`frontend/src/hooks/`) now aggregates the per-span merge into one `SegmentRenderMonitorSegment` per **render batch**, using `ScriptViewResponse.render_batches` (already computed server-side by `get_script_view_payload`, same grouping rule as the real synthesis-time `build_chunk_groups` — no new backend endpoint or schema change needed). A batch's phase is `'failed'` if any member span failed, else `'rendering'` if any member is live, else `'done'` only when every member is done, else `'preparing'`; its progress is the char-weighted fraction across members (same math as M1); `batchSpanIds` is returned alongside so a caller can resolve a batch id back to its member span ids. `SegmentRenderMonitor`, `SegmentBlockRow`, and `SegmentPeekStrip` are unchanged (they were already generic over whatever `segments` array they're handed) — the whole fix is upstream, in what the hook hands them. `QueueItem.handleSegmentRetry` now retries every span in the clicked batch (`api.generateSegments(batchSpanIds[batchId])` — the existing multi-id generate endpoint already accepted several ids in one call; that call **was** the batch-retry capability, it just wasn't being fed a batch's full member list before), not just the batch's leader span. All user-visible copy ("Segment detail (N)" → "Batch detail (N)", per-segment aria-labels, the milestone `aria-live` announcements) now says "batch", never "segment" — new invariant **M6**. |
| 1.11.0  | 2026-07-16 | **`StatusOrb` adopts the §2.7 preparing tier + shows the live segment arc during active renders (PR 01, closes the deferred W-MIX visual-check item).** The chapter `StatusOrb` (`frontend/src/components/ui/StatusOrb.tsx`) previously collapsed to the running spinner (`Loader2 .animate-spin`) for any active job, indistinguishable from the model-load window, and its completed-vs-total segment arc rendered only in idle "stable" states. Two changes: **(1) preparing tier** — a live job whose signal is preparing (same predicate as `useStudioChapter`: `reason_code ∈ {SEGMENT_PENDING, LOADING_MODEL}` or `indeterminate === true`) now renders a distinct dimmed, spinner-less orb: a static center dot + a calm dimmed ring pulse (`orb-preparing-pulse`, opacity band 0.35→0.65, `.is-preparing`), tooltip "Preparing… loading voice model". Precedence sits **above** the running branch (both are true during the load window). Derived entirely from fields already on the `Job` the orb receives — no new signal, no call-site prop threading. **(2) live segment arc** — the existing `--accent` progress arc (real `doneSegments/totalSegments`, a discrete manuscript count, not a fabricated progress value) now also renders during `running` and `preparing`, not only idle; it reflects last-known done/total and does not animate a fake advance during the indeterminate window (no-fabrication holds). Reduced motion: the orb pulse rides the same essential-state exemption as the ScriptView preparing pulse (re-enabled in `base.css` as a calm opacity breathe, no movement). No backend/wire/ETA change. |
...
- **Chapter `StatusOrb` (1.11.0).** The chapter-status orb (`frontend/src/components/ui/StatusOrb.tsx`) mirrors this tier: a live job whose signal is preparing (same predicate — `reason_code ∈ {SEGMENT_PENDING, LOADING_MODEL}` or `indeterminate`) renders a **distinct dimmed, spinner-less** orb (static center dot + calm dimmed ring pulse via `.orb-is-preparing` / `orb-preparing-pulse` — orb-scoped, not the bare `.is-preparing` ScriptView uses on body text), precedence **above** the running spinner branch. The orb also surfaces the real completed-vs-total **segment arc** (`doneSegments/totalSegments`) during `running` and `preparing`, not only idle states — a discrete manuscript count, never a fabricated advance during the indeterminate window. Derived from fields already on the received `Job`; no new signal, no call-site threading.
- **Reduced motion.** The preparing pulse is a pure opacity breathe (no movement). It is an essential state cue (WCAG 2.3.3), so it **survives** `prefers-reduced-motion` as a calm slowed breathe rather than freezing — the segment-span pulse (`.script-line.is-preparing` / `.script-render-group.is-preparing`) and the `StatusOrb` pulse (orb-scoped `.orb-is-preparing`) are both re-enabled in the `base.css` reduced-motion block for this reason.
...
A chapter that renders its segments in parallel (W-PAR) earns an **additive** per-batch visualization: a row of blocks, one per render **batch** (never one per sentence/span — see the owner ruling below), sized to the manuscript and filling in as audio data arrives — the "render monitor" (BitTorrent-style). It sits **beneath** the chapter aggregate `PredictiveProgressBar`; it does **not** replace it. Full UX design (progressive disclosure, power controls, open questions) lives in the plan: [10-phase2-render-monitor.md](../plans/active/parallel-segment-rendering/10-phase2-render-monitor.md). This section is the binding *presentation contract* the production component must satisfy.

**Granularity (owner ruling, 2026-08-26 — `glossary.md` 1.1.0, invariant M6).** The render batch — several consecutive same-speaker spans merged into one synthesis call — is the finest granularity this UI may ever show, in both the block field and the accessible table. A per-sentence row or block ("sentences should not have any part of this") is a defect, not a finer level of detail; it also exposes an internal DB row count nobody outside the codebase should ever see. `frontend/src/hooks/useSegmentInventory.ts` is the one place this aggregation happens — `SegmentRenderMonitor`, `SegmentBlockRow`, and `SegmentPeekStrip` all stay generic over whatever `segments` array they are handed, so a caller that fed them per-span data would reintroduce the defect without this file changing at all.
...
- **M6 (1.12.0)** — **Batch is the finest visible granularity:** every row/block the render monitor shows (block field, peek strip, accessible table, popover, retry action) MUST represent one render batch, never one sentence/span; a batch's phase/progress MUST be aggregated char-weighted across its member spans (§7A, cross-ref M1); a batch-level retry MUST re-generate every member span of that batch, not only its leader (§7A, owner ruling 2026-08-26).
