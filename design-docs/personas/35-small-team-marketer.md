# 35 · "Ben Nakamura" — Small Team Marketer  ☆ INFERRED
...
- The team has no dedicated audio engineer; Ben owns the full workflow from text to publish

## Key workflow moments
- **Render initiation:** Ben queues a chapter render, noting it is a draft; expects a clear "draft" status marker visible in the project and queue panel
...
- **Approval handoff:** Manager listens and approves; Ben needs a way to mark the specific render as approved without re-exporting or relabeling files manually
...
- **F1 — No render versioning or approval state:** The app has no concept of "this is the version under review" vs. "this is the approved version." Ben and his manager track this in Slack, which has broken down twice into shipped mistakes.
...
- **F3 — Voice settings don't carry between projects:** Each new project starts from scratch. Ben re-selects the same voice, the same engine, and the same quality settings every week — a 3-minute ritual that has caused one configuration error.
...
- Export that stamps which render was exported and when, surfaced on re-open so Ben can confirm which file the manager heard
...
- A project note or comment field so Ben can log "approved by Jamie, 2026-06-20" without leaving the app
...
- The queue panel clears completed jobs before Ben checks whether the manager's version is still accessible
