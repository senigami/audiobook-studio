# 11 · "Zara Ahmed" — Sensitivity Reader  ☆ INFERRED

**Identity:** "Zara reviews manuscripts for cultural accuracy and representation before production locks — she needs to flag specific lines with notes that reach the production team without her needing edit access to the text or the voice cast."
...
- **Review export:** At the end of the pass, exports or shares a structured list of flagged segments with her comments, so the producer can act on them without requiring Zara to be in the room
...
- **F1 — No comment or annotation layer:** The chapter editor is a production tool with no native way to attach a reviewer comment to a segment without editing the text itself; Zara currently maintains a parallel spreadsheet with line references, which can drift from the actual project
...
- **F3 — No read-only mode:** Opening the app gives Zara the same interface as the producer; she could accidentally modify text, re-cast a character, or queue a render without intending to; there is no review-only role or view
...
- Flag status tracking: the production team can mark a flag as resolved; Zara can see the response on her next visit
