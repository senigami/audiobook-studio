# 28 · Nontechnical Author  ☆ INFERRED

> **Primary persona.** Decisions that break the Nontechnical Author break the product for the largest share of the intended audience.
...
## Goals
- Upload their manuscript and hear it read back without needing to understand what TTS, plugins, or engines mean
...
- Eventually share the finished audiobook on Amazon or Audible, or just keep it for themselves
...
- Works in the evenings at their kitchen table; sessions are 30–60 minutes before they give up if something is confusing
- Uses the internet to ask questions, but will not read developer documentation or GitHub issues

## Key workflow moments
- **First open:** Looks at the screen, tries to understand what to click first. If the first action isn't obvious, they'll spend 10 minutes clicking around before losing confidence.
- **Import:** Pastes or uploads their manuscript file. Expects it to "just work" — isn't prepared for a "configure your TTS engine" step before they've heard a single word.
- **First render:** Clicks the button they think starts narration. Waits. Wants to understand whether it's working or broken from the progress state — not from reading documentation.
- **Partial failure:** One of their chapters fails midway through. They see a partial result and read it as total failure. Try to start over instead of retrying.
- **Recovery:** Looks for a way to fix the problem. The error message contains words they'd need to Google. They close the tab.

## Top friction points *(INFERRED)*
- **F1 — Plugin jargon before first use:** They see "engine not configured" or "no TTS plugin selected" before they've done anything. They don't know what a TTS plugin is or why they need one to narrate their book.
- **F2 — Partial failure looks like total failure:** A chapter with 3 failed segments and 47 successful ones looks the same as a chapter that completely failed. They re-render the whole thing, losing the 47 that worked.
- **F3 — Progress states don't communicate time:** "Preparing..." has been on the screen for 2 minutes. They don't know if this means "almost done" or "something broke." They refresh the page.
...
- **F5 — Setup flow asks opinions they haven't formed:** The first-run experience asks them to choose between voice options, quality settings, and engine parameters. They haven't listened to a single output yet. They don't have opinions.
...
- **Error messages they can act on**: "This segment couldn't be narrated — try again" instead of HTTP status codes
- A **one-click retry** for failed segments that's visible and clearly labeled
- **Deferred configuration**: let them hear a draft first, surface settings only when they've decided something sounds wrong and want to adjust it
...
- Settings presented in terms they'd need to Google — especially before they've heard their first output
