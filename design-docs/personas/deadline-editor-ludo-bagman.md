# 33 · Deadline Editor  ☆ INFERRED

**Identity:** "A commercial editor under a 48-hour deadline who needs to upload a manuscript, render a draft, and have something playable in time for a client call — and who permanently loses trust in a tool the moment it gives them a false positive status."
...
- Has no patience for onboarding flows; reads no documentation before starting — they expect the critical path (open project → cast → render → play) to be self-evident
- Came to Audiobook Studio under time pressure after their usual tool failed to handle a large file; a colleague recommended it as "fast and local, no upload wait times"
- Tests tools by immediately trying the critical-path workflow; abandons if they hit a blocker in the first 5 minutes; their tolerance for unexplained latency is approximately 8 seconds before they assume the app is frozen

## Key workflow moments
- **Uploading a manuscript and starting a project:** Expects to see a project open with chapters listed within a few seconds of file import; a long spinner with no progress indication after "Upload" is clicked makes them assume the app hung
- **Casting voices quickly:** Expects to cast all characters from a single panel with a voice preview they can play inline — does not want to leave the casting flow to audition voices in a separate library view
...
- **F1 — False positive completion state:** A segment or chapter marked with a green "complete" indicator that has not actually rendered, or whose audio file is empty/corrupt — the Deadline Editor sends this to a client and loses the relationship; false positives are worse than visible errors
- **F2 — Queue latency with no feedback:** The gap between clicking Render and seeing a queue item appear — if it is more than 3–4 seconds with no spinner or intermediate status, they click Render again, queue a duplicate, and then have to untangle two competing jobs
- **F3 — No estimated completion time:** Queue items that show a spinner with no ETA, no segment count progress ("12 of 47"), and no elapsed time — they cannot decide whether to wait or come back in an hour, and the uncertainty is cognitively expensive during a deadline session
- **F4 — Error recovery requires log reading:** Segment failures that surface only as a small error badge, requiring them to open a log panel or detail view to find a retry button — they want Retry surfaced at the segment row level, not behind a secondary panel
- **F5 — No chapter-level audio preview:** No way to play a complete chapter as a continuous stream from within the app — they have to export and open an external player to hear whether character transitions and pacing work, adding 5+ minutes to every review loop
...
- Clicking Render and seeing no response for more than 8 seconds — they will click it again and create a mess
...
- Any session where they cannot tell whether the app saved their work before they closed it
