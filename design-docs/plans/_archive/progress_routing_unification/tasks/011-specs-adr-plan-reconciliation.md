...
- `design-docs/specs/README.md` (router index — read first), `design-docs/specs/live-events.md` (currently **allows dual
  paths** orchestrated + handler-direct — that allowance is now wrong), `design-docs/specs/progress-presentation.md`
...
- **D5 monotonic-floor reconciliation** (`../00-architecture-map.md` D5): §4A/§3 put the *display* floor
  **client-side** (`progressMemory` by `persistenceKey`). The server `enrich` provides monotonic-clamped
  ...
  spec change.)
- `design-docs/decisions/` (ADR home; existing `ADR-0005-websocket-live-events.md` stays — add a new ADR, don't
...
- `../02-plan-reconciliation.md` — the disposition table for the scattered plans
  (`design-docs/plans/final_release/15_progress_confidence_model.md`, `design-docs/plans/v2_progress_tracking.md`,
  `design-docs/plans/implementation/progress_service_impl.md`, `design-docs/plans/implementation/live_event_stream_contract.md`,
  `design-docs/plans/phases/phase_4_progress_and_reconciliation.md`). This task **actually reads each in full**, folds
...
- New ADR in `design-docs/decisions/` (next number): "enrich kernel at the event-builder layer; one RLocked
...
## Steps
1. Read `design-docs/specs/README.md`, `live-events.md`, `progress-presentation.md` §4A in full; diff against the
