...
> **SUPERSEDED by `design-docs/plans/progress_routing_unification/` (shipped 2026-06-18).** `ProgressService` is implemented at `app/orchestration/progress/service.py`; the files listed below exist and are live. `enrich()` is the single contract kernel; routing is single-source at the event-builder layer. Retained for history.
