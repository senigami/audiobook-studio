...
- Spec: `design-docs/plans/book_view_ia_proposal.md` §6.2 (the one unified Chapter Workspace; triple-duty highlight; no Review), §6.3 (chapter-level = the one Workspace, no sub-tabs), §6.5 step 1 ("Merge Studio + Review into one Chapter Workspace — drop the Review tab; reuse the follow-scroll already built").
- Roadmap: `design-docs/plans/book_view_redesign/01-roadmap.md` — Workload 3; `005 ──► 007`; `007 ──► 008,009,013`.
