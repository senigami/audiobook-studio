...
- [ ] **005** — Code simplification — [task file](master_fix_plan/tasks/005-code-simplification.md) · [simplification plan](active/simplification/00_overview.md)
  - [~] FE dead-code ([simplification/02](active/simplification/02_frontend_dead_code_removal.md)): DC-1a extract shared (`VoiceProfileSelect`/`useChapterStatus`/`ResyncPreviewData`/`ChapterEditorTab`) — no payoff while DC-1b blocked, not attempted, DC-1b dead-tree *(gated on 004)*, ~~DC-2 stub-route infra~~ / ~~DC-3b safe independent deletions~~ **done 2026-07-04 (`9d03e483`; found `VoiceModulesPanel` live inside a file slated for wholesale deletion — kept it, surgically stripped only the dead stub)** *(⚠ audit 2026-07-01: the `ChapterEditor`/`ProjectDetail` trees are LIVE-ROUTED and actively developed — coupling grew 4→7+ importers; DC-1b gate must be re-verified before any deletion)*
...
- [x] **Recording cue & persona sample expansion** *(DONE 2026-07-17, worktree `recording-cue-persona-samples`)* — three-stream expansion of the dynamic recording guide, per the owner's ask ("mad lib style… starts generic and as selections are made it can alter sentences or words and themes… library of characters filtered by selections… manual attribute selection should also influence the image… square portrait so prompts that talk about hands or legs need to be fixed"):
  - **Mad-lib cue composer** — new `cueComposer.ts`: slot-based 4-sentence read-aloud passage; Class swaps the speaker framing (5 frames), Tone picks 1 of 8 theme families (all 28 tones grouped; majority wins), Timbre swaps texture adjective/adverb slots (24 pairs), Pace reshapes sentence structure (slow/measured merges, brisk/fast clips, variable mixes), Age adds a light tint, Gender deliberately has zero effect. Composed path now yields a real passage + a theme-matched `sampleText` for TTS previews (was direction-only + null). Deterministic, no random.
  - **Filtered character library** — `ArchetypeQuickPick` accepts the caller's current `attrs` and ranks the archetype table with the shared `scoreArchetype` (now exported): "Best matches" (Match ≥10 / Close match ≥6 badges) above "All characters", nothing hidden; live re-narrowing wired in both `ArchetypePicker` (record mode) and `OverviewTab` (metadata).
  - **Square-portrait image prompts** — `iconPrompt.ts` reframed to "Square 1:1 head-and-shoulders portrait…"; new `iconPromptFragments.ts` maps class/age/gender/tone/timbre → visual descriptors so manual selections influence the image on both the archetype and composed paths; 29 of the original 39 `appearance_description`s rewritten portrait-safe (hands/legs/boots/posture removed) across `voice_archetypes.json` + regenerated `.csv` + `recordingArchetypes.ts` (3-way parity verified programmatically); a regex-denylist unit test prevents regression. Backend `app/domain/voices/recording_archetypes.py` reads the same JSON, no change needed (6/6 backend tests green).
  - **Character library expanded 39 → 103** *(same day, owner follow-up)* — owner flagged the 39 curated archetypes skewed toward job-title reskins (explicit example: no vampire) and asked for a genuinely diverse, uncapped roster to aid character development. Designed 64 new archetypes across 8 genre buckets (gothic/horror, sci-fi/space, fantasy/myth creatures, folklore/trickster spirits, historical/period, pulp/superhero/crime, adventure/RPG archetypes, post-apocalyptic/cyberpunk) — taxonomy-valid tags (exactly 3 tones + 3 timbres each, from the existing 28/24-value vocabulary) assigned directly by the orchestrator to guarantee validity and avoid overlap; prose (appearance_description, recording_prompt, sample_text, direction_note) authored by 8 parallel writing agents against a shared schema, portrait-safety constraint, and an explicit no-caricature instruction for archetypes tied to real-world folklore/history (Kitsune, Naga, Viking, Samurai, Egyptian, Athenian, etc. — fluent dignified English, no accent-spelling). All 64 validated programmatically before merge (schema shape, taxonomy membership, exactly-3 tone/timbre counts, no duplicate names, portrait-safety denylist) — zero issues found. Merged into `voice_archetypes.json` (source of truth) and regenerated `.csv` + `recordingArchetypes.ts` from it; one denylist false-positive ("sitting" in Caped Superhero's mask description) caught by the broader test-file regex and fixed pre-merge. Updated the hardcoded `39` in `iconPromptFragments.test.ts`'s length assertion and doc comments in `recordingPromptSuggester.ts`/`iconPrompt.ts`/`recording_archetypes.py` to 103.
  - Gate: 424/424 targeted frontend tests, tsc clean, eslint clean (3 pre-existing-pattern warnings) before the expansion; re-gated after. Triage note: `design-docs/personas/` (46 product personas) is a review-lens library, intentionally NOT merged with the voice-character library.
  - **Adversarial re-review (fable) caught one real defect**: Cybernetic Mercenary and Cyberpunk Street Samurai (both new) had identical dominant_tones/dominant_timbres sets — a genuine near-duplicate archetype, not a stylistic nit. Fixed directly: Street Samurai's tags reworked (serious/intense/confident + crisp/low/clear, dropped `deadpan`/`robotic`) and its prose rewritten around a distinct "honor-code blade" premise. Re-verified: zero exact tag-signature collisions remain across all 103. Everything else — taxonomy validity, duplicate names, portrait-safety denylist, 3-way parity, backend impact, and a caricature check on the 9 folklore/history-adjacent entries — came back clean on independent re-check.
  - **Image-prompt follow-up, settled across two owner messages** *(same day)*: (1) upgraded the composed (no-archetype-match) image prompt from a flat fragment-join to a real mad-lib composition — new `imagePromptComposer.ts`'s `composeImageDescription()` reuses `iconPromptFragments.ts`'s existing vocab but varies sentence STRUCTURE by class and by tone/timbre count (0/1/2/3+), mirroring `cueComposer.ts`'s philosophy; wired into `iconPrompt.ts`'s composed branch, archetype-match branch untouched. (2) new `simpleArchetypeIcon.ts` — the canonical, reusable flat/generic default-portrait prompt + filename-slug generator per archetype, distinct from the existing detailed `buildIconPrompt()` a user copies to generate their own distinct image. Went through two rounds of correction before landing on the owner's actual intent: first pass wired an always-visible "copy visual prompt" button into `ArchetypeQuickPick.tsx`; owner clarified this should be a dev/owner tool for pre-generating static default images the picker shows to everyone, not an end-user feature — reverted; owner then clarified further that a dev-mode-gated copy button is fine, and asked for the whole thing to be durably reusable as more archetypes get added later. Final shipped design: `useDevMode()` (existing localStorage-backed hook, same pattern as other dev-only affordances in this repo) gates the copy button; `ArchetypeQuickPick`'s `LibraryRow` gained a `PortraitThumbnail` that looks up `frontend/public/archetype-portraits/<slug>.png` and silently renders nothing until that file exists (`onError` hide — nothing breaks pre-images); new checked-in `frontend/scripts/exportArchetypePortraitPrompts.ts` (run via `npm run export:archetype-portraits`, `vite-node`) regenerates `design-docs/reference/voice-archetypes/default-portrait-prompts.md` — every archetype's exact slug + prompt, imported directly from the same source functions (no duplicated logic to drift) — so future archetype additions stay visually consistent via one command, not a one-off script. A genuine cross-turn file race occurred (the orchestrator's manual correction and the still-running background agent's writes landed out of order twice) before the agent's actual completion notification arrived; resolved once, cleanly, after confirming completion.
  - Re-gated after settling: 528/528 targeted frontend tests, tsc clean, eslint clean (3 pre-existing-pattern warnings).
  - **New convention adopted, applies beyond this feature**: use the existing `useDevMode()` hook (`frontend/src/utils/devMode.ts`, doc comment expanded 2026-07-17) as the default gate for contributor-only tooling, in-progress features, and beta rollouts — not just debugging. Reach for a real feature-flag/permission system only when a surface would be actually harmful or misleading if exposed, not merely "not ready yet."
  - **Follow-ups** *(owner-side, tracked so they don't get lost)*:
    - [ ] Owner generates the 103 default portrait images from `design-docs/reference/voice-archetypes/default-portrait-prompts.md` (one prompt + exact filename slug per archetype) using an external image generator, and drops the resulting `<slug>.png` files into `frontend/public/archetype-portraits/` — the picker's `PortraitThumbnail` picks them up automatically, no code change needed. Rerun `npm run export:archetype-portraits` (frontend/) to regenerate the sheet whenever new archetypes are added later.
    - [ ] Owner live-verifies the character library end-to-end after merging (this worktree had no local Python venv, so the backend couldn't run here for a live click-through this session — everything shipped rests on the 531-test suite + three independent adversarial reviews, not an in-browser pass): the picker's ranking/narrowing and dev-mode-gated copy button in both `ArchetypePicker` (record mode) and `OverviewTab` (metadata editor); the mad-lib composed recording-cue passage and image-description prompt for voices that don't match a curated archetype; the square-portrait `buildIconPrompt()` output end-to-end for a few real voices.
  - **Third adversarial pass (fable) found 2 real grammar defects in the new mad-lib image composer** — both confirmed by tracing real function output, not speculative: (F1) the human/no-class opening's "with" fusion assumed every age/gender fragment starts with "with"; `AGE_VISUALS.ageless`/`.senior` don't, producing "with of indeterminate, ageless appearance" — fixed with a `joinWithFragments()` helper that only fuses when the first fragment actually starts with "with", appositive otherwise. (F2) the 2/3+-timbre sentences used a singular "X meets Y" verb regardless of the first fragment's grammatical number; ~13 of 24 `TIMBRE_VISUALS` entries are plural noun phrases, producing "features meets…" — fixed with a number-agnostic "X and Y meet…" compound subject. Also fixed a flagged usability trap before any portrait images exist: `slugifyArchetypeName()` didn't fold diacritics, so "Victorian Séance Medium" (the one non-ASCII archetype name) slugged to the separator-mangled `victorian-s-ance-medium` — now normalizes to `victorian-seance-medium` via NFD decomposition + combining-mark strip. Re-gated: 531/531 targeted frontend tests, tsc clean, eslint clean. Two items explicitly flagged as pre-existing/out-of-scope, not touched: `buildIconPrompt` double-periods when `meta.description` already ends in "."; "a death itself" (Grim Reaper's simple prompt) reads slightly oddly.
