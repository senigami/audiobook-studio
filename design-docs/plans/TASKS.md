...
- [ ] **015** — Multi-job render-monitor rows *(NEW, flagged by Fable sign-off 2026-07-11 — not yet planned)*: extend `ActivityPage.tsx`/`GlobalQueue.tsx` so EVERY concurrently-rendering job gets its own inline monitor/peek-strip row, not just the first active job found — this is the actual owner-stated intent this session ("if I had 2 chapters rendering at the same time... I would have a strip underneath each progress bar"), which 008-011 only partially delivered (single-job scoping). Needs a plan (`plan-architect`) before execution — likely: hoist `useSegmentInventory` per-job instead of singleton, render one `SegmentPeekStrip`/`SegmentRenderMonitor` per active row in `GlobalQueue`'s list, and fix the stale comment in `ActivityPage.tsx` claiming 010/011 already handle "choosing among several" (they don't). Also flagged in the same pass: `useSegmentInventory.ts` refetches `GET /script-view` on every `active_segments_map` identity change (roughly once per progress tick) — bounded today by the `devMode` gate, but should be fixed (fetch once per `chapterId`, merge the live map client-side) before that gate comes off.
