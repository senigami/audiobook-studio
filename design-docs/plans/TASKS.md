...
## W-PAR — Parallel segment rendering *(active — 001/003/004/005 done, 002 done + LIVE-WIRED (the enable-gate), 006/007 pending; W-MIX-LA 007 gate CLEARED 2026-07-02)*
...
- [ ] **007** — ETA under parallelism + off-by-default toggle + spec reconciliation — [task 007](active/parallel-segment-rendering/tasks/007-eta-toggle-and-specs.md) *(bracketed throughput ETA; cap-default-1 toggle; bump queue-jobs/system-architecture/data-model/live-events/progress-presentation; final invariant gate)*
  - [ ] **Fable merge-gate findings on the 2026-07-03 cap fix (commit 7dd218aa), deferred here as this task's actual scope** — the registry in `resources.py` is keyed by `engine_class` string, not `engine_id`: two different engines resolving to the same class (e.g. two GPU-class engines) with different `max_concurrent_workers` would now silently converge to the larger one's cap via the grow-only fix, since nothing enforces a per-engine ceiling independent of the shared class semaphore. Not live today (only XTTS resolves to "gpu"; voxtral/mixed both resolve to "cloud" at cap=1) — a latent risk for the next GPU-class or cpu_heavy-class plugin. Consider per-`engine_id` admission independent of the class gate as part of this task's "final invariant gate." Also consider hard-pinning the `"exclusive"` class to reject growth above 1 (today only `ResourceClaim.exclusive_claim()` ever constructs it, always at cap=1 — safe in practice, not enforced).
