...
- [ ] **L-NS 006** — Library: "Continue" section (depends on 005 — status data now partially exists; Studio/Published still unresolved, same blockers as 005) — [plan](active/north_star_screen_parity/tasks/006-library-continue-section.md)
...
- [ ] **L-NS 012** — Demo-side cleanup: Engines Module-Settings tab, orphaned `ManuscriptPane`, `studio.tsx` note (Part B now unblocked — task 010's decision is settled and executed) — [plan](active/north_star_screen_parity/tasks/012-demo-side-cleanup.md)
- [ ] **L-NS 013** — Refresh terminology in the R1-18 owner-validation checklist (best done now that 008/009/010 have all landed) — [plan](active/north_star_screen_parity/tasks/013-refresh-validation-checklist-terminology.md)
...
- [~] **HuggingFace voice browse + upload** — [plan](active/v2_huggingface_voice_interface.md) *(LIVE as of 2026-07-03 — real `huggingface_hub` network client, router, and frontend Discover tab all wired; see per-item notes for what's genuinely done vs. still deferred)*
  ...
  - [x] Export: bundle generator → `.asvoice.zip` for manual upload *(DONE 2026-07-03 — `POST /api/voices/huggingface/export` wraps `export_hf_voice_bundle` for an installed voice by id; still intentionally separate from `app.domain.voices.bundles.export_voice_bundle`, the existing on-disk `.voice.zip` exporter — the two bundle formats/use-cases were kept distinct rather than unified, matching the original plan's `.asvoice.zip` vs `.voice.zip` split. **Gap identified 2026-07-04**: the exported bundle only contains `voice.json` + `samples/preview.mp3` — no `icon.png`, no `README.md`, no engine assets — closed by [huggingface_voice_upload plan](active/huggingface_voice_upload/README.md) task 002.)*
  - [~] Upload to HF: push loose files via user token; auto-set `as-*` tags *(DONE 2026-07-03 — `POST /api/voices/huggingface/upload` extracts the exported bundle and calls `HFHubClient.upload_files` (`create_repo` + `upload_file` per file + a model-card tag push); requires a configured token, 422s otherwise. **Gap identified 2026-07-04** (deep-research + code audit): the per-file `upload_file()` loop uses `path_in_repo=Path(file_path).name`, which **flattens directory structure** — `samples/preview.mp3` lands as `preview.mp3` at repo root, breaking the widget's expected path and any future `assets/<engine_id>/` nesting — plus it's N sequential commits, not atomic. Fix + full HF repo-spec alignment (icon, generated README with `widget.output.url`, engine assets, `huggingface_hub.upload_folder()` atomic push) tracked in [huggingface_voice_upload plan](active/huggingface_voice_upload/README.md) (5 tasks; task 004 needs an owner decision on variant-vs-engine_id scoping before it ships). Implementation research: [v2_huggingface_upload_implementation.md](reference/v2_huggingface_upload_implementation.md).)*
