...
> **SUPERSEDED by `design-docs/specs/live-events.md` (spec_version 1.5.2, 2026-06-18).** The authoritative live-event contract now lives in that spec. The dual-path allowance (orchestrated + handler-direct emitting independently) is retired; both producers enrich via `ProgressService.enrich()` then build via the event builders in `app/api/contracts/events.py`. Retained for history.
