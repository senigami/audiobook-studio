...
- **006 — `WaveformTape` renderer + browser peak provider.** ✅ DONE — `frontend/src/app/layout/WaveformTape.tsx` ports `MockWaveTape`'s render/interaction (fixed-grid sampling §5.3, paged **and** moving motion, click+drag scrub, m:ss ruler, keyboard ±5s) bound to the **single `<audio>`** via `playerBus.seek()` (no `reportTime` call — that stays PlayerBar-owned). `usePeaks(audioUrl, audioEl)` decodes via `AudioContext`/`decodeAudioData` → downsamples to `PEAKS_COUNT` (4000) buckets; never touches the passed `audioEl` (single-owner preserved — verified by grep and a dedicated test). Moving mode polls `audioEl.currentTime` via a rAF loop for smooth sub-tick scrolling; `prefers-reduced-motion` forces paged at mount. Exports `TAPE_ZOOM_PRESETS_SEC`/`TapeZoomPreset`/`PEAKS_COUNT` for task 007. Not yet mounted in `PlayerBar.tsx` (task 008). Vitest: 18 tests, including R1 revert-checked fixed-grid/paged-snap/reduced-motion assertions.
