...
**Cover-slider visual (port from `MockTapeControls.tsx:38–84`):** same CSS class names as the North-Star mock's Library size slider (`ns-size-control`, `ns-size-slider-wrap`, `ns-size-track`, `ns-size-tick`, `ns-size-slider`). **Correction (2026-07-04, review-ratchet):** these classes are only defined in the demo mock's `frontend/src/demo/stages/siteMockup/mockup.css` — there is no live "Library" component or stylesheet in the shipped app, so this is NOT free reuse of existing live CSS. Task 009 must port the relevant `ns-size-*` rules from `mockup.css` into the live app's stylesheet, keyed to the exact hook classes `WaveformTapeZoom.tsx` actually renders (`ns-size-glyph tape-zoom-glyph-hit`, `ns-size-slider tape-zoom-slider-hit`, `ns-size-tick tape-zoom-dot[--active|--disabled]`) — not a `.tape-zoom-btn` class, which the shipped component does not use. Until 009 lands, the control renders functionally but visually unstyled. The slider `value` = index into `TAPE_ZOOM_PRESETS_SEC` (0 = 8 s = most zoomed in; 4 = 120 s = most zoomed out).
