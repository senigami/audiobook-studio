...
**Feeds from:** a design reference (AR-1, 2026-07-18).
...
2. **One-line integration inside the chokepoint**: `resolve_effective_cap`
   ...
   manifest-ceiling clamp, not before — **plan review caught a real arithmetic bug here**:
...
5. **Sampling-failure fallback**: freeze the current penalty at whatever it already is — never reset
   ...
   "fail-open-reset"; plan review flagged this wording as ambiguous) —
   per the no-fabrication principle. After 3 consecutive failures, flag `sampling_ok: false` visibly.

## Open questions — LARGELY RESOLVED by review, with one genuine disagreement to escalate

- **Can the Studio process read CUDA memory at all? NO — independently confirmed.**
...
- **Genuine disagreement on the correct transport (escalate — this is exactly the kind of split
  independent review exists to surface, not to average):**
  - **Option A**: ride the TTS server's existing `/health` heartbeat (the watchdog already polls
    it; the xtts-env subprocess holds the real CUDA context, so sampling there is free and accurate).
  - **Option B**: call NVML/`nvidia-smi` directly from the Studio daemon, reading global board memory
  ...
  - **Both options agree**: don't sample via in-process `torch` in Studio. They disagree on subprocess-poll
...
- **Goal-honesty gap (F3/F4 — must be stated plainly, not silently scoped around):** the
...
- **ETA-jitter regression risk (F5 — needs an explicit decision, not left implicit):** since
...
1. **Implement the sampling transport per the owner's pick** (the `/health`-heartbeat option vs.
   the NVML/`nvidia-smi`-from-Studio-daemon option — both are now fully specified; this is no longer a
   research task, just an implementation choice). Do NOT implement in-process `torch.cuda` sampling
   in the Studio daemon — independently ruled out.
...
7. **Full regression**: existing scheduler/resources tests unaffected when the monitor reports 0
   ...
   regression risk a generic "scheduler/resources tests" pass could miss.
