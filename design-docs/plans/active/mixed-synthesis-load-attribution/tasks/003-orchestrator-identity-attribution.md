...
> **⚠ LOCKED by [001-findings.md](001-findings.md) — read it first.** The diagnostic found that once 002 makes the load line *arrive*, the remaining work is narrower than this file's original framing: **(1) "Bug 2"** — confirm/fix that `_resolve_active_engine_for_matching` (`:561-585`) yields the per-segment engine (`"xtts"`) for a mixed job so the guard at `:715-756` (`matched_marker_engine == active_engine_id and _active_engine_has_specific_activity_marker(active_engine_id)`) actually fires; if the mixed `script` lacks per-group `engine`, populate it or relax the guard to fire when `matched_marker_engine` has a specific marker and `active_seg_id` is set. **(2)** keep warm/cloud silent (free — no load line is emitted for them). The ambient heuristic does NOT need wholesale replacement (correcting this file's original "retire it" framing); the fix is delivery (002) + correct active-engine resolution. Verify the exact resolution with the owner G0 trace or by reading how the mixed `script` is assembled.
