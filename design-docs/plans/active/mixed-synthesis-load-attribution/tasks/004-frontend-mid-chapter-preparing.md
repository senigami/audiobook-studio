...
**Workstream:** W-MIX-LA · **Depends on:** 003 (emits the correctly-attributed frame) · **Blocks:** 007 · **Status:** FIXED (2026-06-26) — reopened then resolved: `PredictiveProgressBar` now honors `indeterminate` (prop threaded from overlay via `progressBarContracts` + `ChapterHeader`); `preparingIndeterminate = isPreparingStatus(status) || indeterminate===true`; predictive lane end forced `null` while indeterminate (no creep/jump); 120s fallback seed suppressed for load windows; span pulse already worked (test added). typecheck+lint clean, suites green. — store *carries* `indeterminate` (verified live), but `PredictiveProgressBar` doesn't *act* on it: it gates the preparing pulse + predictive-lane suppression on `status === 'preparing'` only (`predictiveProgressBarHelpers.ts:16`, `PredictiveProgressBar.tsx:247`), so a mid-chapter `status:'running' + indeterminate:true` load frame runs the 120s fallback lane (creep → ~18% jump-ahead → no pulse → chapter never pauses). FIX: honor `indeterminate===true` (pulse + suppress lane) regardless of status, threaded to the chapter-header + segment bars. Original carrier work below stands.
> **Earlier (carrier work, complete):** `indeterminate` on the segment frame (`build_segment_progress_event` + service.py threading; `reason_code=LOADING_MODEL` is stripped from `segments.progress`, so `indeterminate` is the durable signal); `live-jobs.ts` scope-gate relaxed (R-C). Typecheck + 101 FE / 252 BE tests green.
