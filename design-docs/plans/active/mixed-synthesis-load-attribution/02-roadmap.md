...
| [005](tasks/005-chapter-level-preparing.md) | *(optional)* Chapter/queue-level preparing | P-C,P-F | 003 | Surface the load window at the chapter/queue level (indeterminate styling / reserved time). Owner 👁 decision on semantics (R-E). |
| [006](tasks/006-load-aware-eta.md) | *(optional)* Load-aware ETA from history | P-G,P-H | 003 | Fold recorded `model_load_seconds` into the forward ETA (cold-vs-warm aware). |
...
- **005 / 006** are optional polish, parallelizable after 003, gated by owner appetite (and R-E decision for 005).
- **007** lands specs + the G0 re-check; if 005/006 are deferred, 007 still runs after 004.
