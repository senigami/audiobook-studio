...
> **Hugging Face Hub**. The concrete on-Hub shape is `design-docs/plans/v2_huggingface_voice_repo_spec.md`;
> the metadata/tags come from `design-docs/plans/v2_voice_tag_taxonomy.md` and
> `design-docs/plans/v2_voice_metadata_and_casting.md`. **Voices live on Hugging Face; engines live on
> GitHub** (`design-docs/plans/v2_engine_bundle_github_distribution.md`) — this document is voices only.
...
`design-docs/plans/v2_engine_bundle_github_distribution.md`.
...
- Map an imported HF artifact onto the canonical voice model (`VoiceProfile` +
  ...
  `design-docs/plans/v2_voice_metadata_and_casting.md`.
...
- Import produces a `VoiceAsset` via the engine contract's `build_voice_asset(...)` /
  `VoiceAssetBuildRequest` defined in `design-docs/plans/v2_voice_system_interface.md` §3.
- Bundles that follow `design-docs/plans/v2_huggingface_voice_repo_spec.md` are read natively; imported
  voices land in `tts_voices/`.
- Imported voices get presentation + structured metadata from
  `design-docs/plans/v2_voice_metadata_and_casting.md` + `design-docs/plans/v2_voice_tag_taxonomy.md`, including
...
4. **Import.** Download the bundle. If it follows the **Audiobook Studio voice bundle
   shape** (`design-docs/plans/v2_huggingface_voice_repo_spec.md`), Studio reads `voice.json` and
...
- The gap that once existed between this spec and the shipped implementation (flattened
  directory structure, no icon/README in the exported bundle, no engine-asset inclusion) has been
  closed — see `design-docs/specs/voice-bundles.md`'s 1.7.0/1.8.0 changelog entries for what
  `export_hf_voice_bundle`/`HFHubClient.upload_files` do today.
...
- `design-docs/plans/v2_huggingface_voice_repo_spec.md` (the on-Hub bundle shape)
- `design-docs/plans/v2_voice_tag_taxonomy.md` (tags)
- `design-docs/plans/v2_voice_metadata_and_casting.md` (metadata & casting)
- `design-docs/plans/v2_engine_bundle_github_distribution.md` (engines — separate host)
- `design-docs/plans/v2_voice_system_interface.md`, `design-docs/plans/implementation/voice_engine_impl.md`
