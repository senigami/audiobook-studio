...
## Decisions needed (owner input required before certain tasks can execute)

These are real forks found during research — this plan does not guess at them.

1. **Backups tab: stub or real, and where?** The demo's `Backups` tab is a fully working
   feature (list/restore/create). The live app's standalone `Backups` tab
   (`BackupsStage.tsx`) is an explicit placeholder ("coming in Phase 2 — use Publish tab") while
   the *real* backup functionality already lives inside the live `Publish` tab
   (`ProjectBackupsPanel` in `PublishStage.tsx`). Options: (a) remove the standalone `Backups` tab
   entirely since Publish already covers it — matches what the live app actually does today, just
   without the confusing stub; or (b) move real backup functionality into its own `Backups` tab to
   match the demo, and slim `Publish` back down to assembly-only. See task `009`.
2. **Contents tab: chapter board, or chapter table + inline editor?** The demo's wired `Contents`
   tab is a slim chapter board with no inline text editor (plus a cross-book bookmark panel the
   live app doesn't have at all). The live `Contents` tab is a chapter table with an inline
   `ChapterTextPanel` editor next to it. Research found the demo's *own* file has an orphaned,
   unused `ManuscriptPane` export that actually matches the live design closely (editor + lock
   logic + row actions) — suggesting the live design may be the settled, intended one and the
   demo's wired `ContentsPane` just never got the equivalent upgrade. Options: (a) treat live as
   correct, update the demo's wired tab to match (separate, smaller demo-only task, not blocking
   this plan); or (b) treat the demo's slimmer board as the intended target and remove the inline
   editor from live's Contents tab (it would need to live somewhere else, e.g. only in the Chapter
   Workspace). See task `010`.
