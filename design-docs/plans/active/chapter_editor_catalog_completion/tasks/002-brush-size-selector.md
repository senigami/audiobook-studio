...
- **REUSE, DO NOT REINVENT** the word-boundary **snapping** algorithm shipped 2026-07-17 (PR #143,
  its plan folder since retired): `ScriptView.tsx` exports `snapOffsetToWordBoundary(text, offset, boundary)`
  (authoritative Python twin `_snap_offset_to_word_boundary` in `app/domain/chapters/operations.py`).
  Reuse the exported `snapOffsetToWordBoundary` for word-boundary math here (INV-1: don't invent a
  second algorithm for the same "find word edges" job) — see Steps below for exactly how.
...
9. Append a `design-docs/code-map/queue/` changelog entry.
...
- [ ] `design-docs/code-map/queue/` entry added.
