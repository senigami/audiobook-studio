...
- **CHECK, DO NOT ASSUME** `design-docs/plans/active/span_word_boundary_snapping/00-overview.md` and its `tasks/001-frontend-word-boundary-snapping.md` — a *sibling, still-`pending`* plan that specifies a word-boundary **snapping** algorithm for a different feature (snapping a drag-selection's raw endpoints to the nearest word edge). As of this research it has **not been implemented** — `ScriptView.tsx` has no `snapOffsetToWordBoundary` function today (confirmed by reading the full file). If it has landed by the time you start this task, reuse its exported function for word-boundary math (INV-1: don't invent a second algorithm for the same "find word edges" job) — see Steps below for exactly how.
