...
**Aggregate derived from the segment set (spec M1).** The chapter % shown on the aggregate bar above the field MUST be the char-weighted sum over the same segments (`(Σ done.chars + Σ rendering.chars × fill) / Σ total.chars`), never an independent counter — so the strip and the bar cannot drift (the aggregate honestly stalls during `preparing` and dips on a fail→retry). This mirrors the char-weighted progress rule (progress-presentation §4A.9 / B9). **Still followed as of 2026-08-26 (issue #231):** the char-weighted % bar was deliberately left unchanged when the discrete count fix landed — only the "N of M" LABEL now uses real render-batch counts when available (never the % math). This doc's "segment" throughout uses the pre-ruling meaning (a sentence-level row, per `design-docs/specs/glossary.md`'s terminology ruling, not yet executed as a rename) — not yet updated, tracked as part of #232's broader touch list rather than fixed here.
