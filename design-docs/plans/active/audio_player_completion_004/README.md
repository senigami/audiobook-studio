...
This plan closes out [`design-docs/plans/master_fix_plan/tasks/004-audio-player-completion.md`](../../master_fix_plan/tasks/004-audio-player-completion.md) in full. It **supersedes** the remaining unshipped tasks (008–012) of the now-removed `audio_player_waveform_scrubber/` plan — that folder's tasks 001–007 (mock prototype, spec 1.6.0, `fitsLegibly`, `WaveformTape`/`WaveformTapeZoom`/`WaveformTapeMinimap`) were already shipped and were load-bearing context for this plan; it was deleted in the docs cleanup once fully superseded (see `wiki/Changelog.md`), so 008–012 no longer exist to be read as stale drafts.
