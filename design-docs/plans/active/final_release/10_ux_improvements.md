...
- [~] **U3 (M). Semantic type scale.** *(Design APPROVED by owner 2026-06-12 via styleguide typography section.)* `tokens.css` has zero type tokens; 11 ad-hoc font sizes exist (0.625rem–2.75rem). Add a 6-step `--type-*` scale (title 1.5rem/700, headline 1.125/600, body 0.9375/400, callout 0.875/400, caption 0.75/500, micro 0.6875/600), replace inline sizes in `ProjectLibraryPage`, `ChapterHeader`, `ScriptView`, `ConfirmModal`; delete anything below micro (the 0.65rem/0.625rem labels in `components.css:308` are unreadable).
  ...
  **Status corrected 2026-07-16:** the `--type-*` scale (`--type-title`/`-headline`/`-body`/`-callout`/`-caption`/`-micro` plus `--type-display`/`-large-title`/`-reading` extensions) already exists in `frontend/src/theme/tokens.css`, matching `design-docs/specs/design-system.md:234-251` — the token layer is done. What remains open is *real-page adoption*: only ~11 of 615 `var(--type-*)` references repo-wide are in real pages today, the rest are in the demo tree. That adoption sweep is a separate, larger migration tracked under `design-docs/plans/reference/site_redesign_rollout/` — explicitly out of scope for this PR.
...
- [ ] **U9 (M). One button/input system.** Kill `GhostButton`'s JS hover state (conflicts with `.btn-ghost:hover` CSS — different visuals per call site); consolidate the four input styles to one `.input` class (tracked in docs 06/09 R4); sweep raw `<input style={{…}}>` usages (e.g. create-project modal).
  **Status corrected 2026-07-16 (attempted, reverted after adversarial review):** the JS hover state was removed and CSS `.btn-ghost:hover` relied on instead, but `GhostButton`'s inline `style` prop sets `border`/`color`/`background` explicitly on every render — inline styles win over CSS pseudo-classes without `!important`, so this silently made hover a no-op across every non-active `GhostButton` (icon buttons in nav/toolbars). Reverted to the JS `isHovered` approach with a comment explaining why JS is required here; `GhostButton` is functionally unchanged from before this PR. De-JS'ing this component correctly would require moving `isActive`'s state-driven styling to a CSS class too (so no inline style shadows the hover rule) — that's a small but real behavior-adjacent change, out of scope for a behavior-preserving cleanup PR. The four-way input-class consolidation (`.form-input` duplicated in `misc.css`/`shared.css`, plus `.input-group input`, `.inline-edit-input`, `.metadata-field-input`) remains open and is redesign-scale — deferred. **U9 remains open.**
