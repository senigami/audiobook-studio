...
> Supersedes the draft in `design-docs/plans/v2_voice_metadata_and_casting.md` (which remains as
...
shape to the full bundle spec already defined in `design-docs/specs/voice.schema.json`. Nothing in
that schema is new; this plan makes existing profile data migrate into it.

**Full example** (the `gravel-road` demo voice from `design-docs/specs/voice-bundle-template/`):
...
The `attributes` object inside `voice.json` maps exactly to `design-docs/plans/v2_voice_tag_taxonomy.md`.
Cardinality and valid values are enforced by `design-docs/specs/voice.schema.json`.
...
1. **Hard filters** (eliminate before scoring): `languages` must include `project_language`.
   ...
   > Note: `design-docs/plans/v2_voice_tag_taxonomy.md` §4 labels `class` a "Hard filter (match character
   ...
   > `design-docs/plans/v2_voice_tag_taxonomy.md` §4 to match when executing this doc.
...
fallback" principle in `design-docs/plans/v2_voice_system_interface.md` §8).

### 2.4 Decisions made

The items below resolve the open assumptions in `design-docs/plans/v2_voice_metadata_and_casting.md`.
...
- [ ] **A1. Confirm `design-docs/specs/voice.schema.json` is current.**
  ...
  _Acceptance: `python -m jsonschema --instance design-docs/specs/voice-bundle-template/voice.json --schema design-docs/specs/voice.schema.json` exits 0._

- [ ] **A2. Add schema validation to the backend voice loader.**
  On load of any `voice.json`, validate against `design-docs/specs/voice.schema.json`. **The loader
...
- [x] **F1. Update `docs/user-guide/voice-tags-icons.md` (Phase 12 stub) with the tag taxonomy table, icon requirements, and a walkthrough of the tag editor.**
  _Done 2026-06-12. Created `docs/user-guide/voice-tags-icons.md` with the full taxonomy table (from `design-docs/specs/voice-taxonomy.json`), icon upload requirements (1:1, PNG-normalized, 422 on non-square), and a step-by-step walkthrough of the NarratorCard "Not tagged" badge, Edit Metadata modal, attribute selects, free tags, and icon crop flow._

- [x] **F2. Update `design-docs/specs/voice.schema.json` docstring/description fields if any cardinalities were clarified during implementation.**
...
- [ ] **G1. Taxonomy v2 vocabularies** in `design-docs/specs/voice-taxonomy.json` (version bump + changelog):
...
- `design-docs/plans/v2_voice_tag_taxonomy.md` — taxonomy v1.0 (source of truth)
- `design-docs/plans/v2_voice_metadata_and_casting.md` — original draft (background reading)
- `design-docs/specs/voice.schema.json` — schema (source of truth)
- `design-docs/specs/voice-bundle-template/voice.json` — canonical example
- `design-docs/plans/v2_huggingface_voice_repo_spec.md` — HF bundle shape
- `design-docs/plans/v2_voice_system_interface.md` — "no silent fallback" principle
