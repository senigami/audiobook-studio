...
**Note:** This item is tracked in sibling doc `design-docs/plans/02_*` (SDK migration). Do not re-plan here — reference only.
...
- [ ] File the task in `design-docs/plans/master_agnostic_tasks.md` under a Phase 13 heading.
...
- [ ] File in `design-docs/plans/master_agnostic_tasks.md`: split into `timelineDebug.ts` (to be retired) and `runtimeMetrics.ts` (to be retained).
