...
- [x] **8. CI / release checklist.** *(done 2026-06-11 — steps wired into doc 08 §6e)* Add both scripts to the release checklist (doc 08) so they run at each release: `npm -C frontend run sync:showcase-tokens` (regenerates token region in `docs/v1.html`) and `npm -C frontend run build:demo` (rebuilds the interactive demo into `docs/demo/`). Stale demo or drifted tokens = silent misrepresentation.
