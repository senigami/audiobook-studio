...
> are separate and live on Hugging Face (`design-docs/plans/v2_huggingface_voice_repo_spec.md`). Builds
> directly on the plugin contract in `design-docs/plans/v2_plugin_sdk.md` and the namespace rename in
> `design-docs/plans/master_agnostic_tasks.md`. Research sources at the bottom.
...
Per `design-docs/plans/master_agnostic_tasks.md` ("Deferred Phase: Namespace Rename"):
...
Extends the existing plugin layout (`design-docs/plans/v2_plugin_sdk.md`):
...
5. **Register** through the engine registry; the TTS Server loads it (engine code runs in
   the TTS Server subprocess, never the Studio process — `design-docs/plans/v2_voice_system_interface.md`
   §10).
...
- Builds on: `design-docs/plans/v2_plugin_sdk.md`, `design-docs/plans/v2_voice_system_interface.md`,
