...
design-docs/specs/design-system.md                           (§6 StatusOrb + PredictiveProgressBar entries; spec_version bump)
```
...
**Step 6 — Update `design-docs/specs/design-system.md`**
...
- Cross-reference `design-docs/specs/progress-presentation.md` in the §6 note — confirm this task does not alter any behavior described there (it doesn't: only visual layer added, no props/contracts changed).
...
**TDD note:** step 0 requires the new StatusOrb + terminus-icon tests to be written and confirmed RED before any implementation. After implementation, confirm GREEN. Per `design-docs/specs/testing-standards.md` R1.
...
- Do not move `StatusOrb` rendering to CSS classes (that is a P5 / `design-docs/plans/simplification/` concern).
