...
| **P5** | `tasks/005-cleanup.md` | Extract remaining inline-styled primitives (SearchableSelect/ColorSwatchPicker/VoiceDropzone/ConfirmModal) to token classes (overlaps `design-docs/plans/simplification/`); tokenize ~15 legacy hardcoded colors (grandfather ColorSwatchPicker palette). **Optional:** the full `--accent`→`--action-primary` rename (94 files) — only if owner wants the alias retired (**R3**). | Med (cleanup; rename is high-ripple) | Stabilization after the visible system is final. |
