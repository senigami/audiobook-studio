...
- Read for intent (do not duplicate): `design-docs/plans/book_view_ia_proposal.md` §6.7 (three tiers), §9 answer **A20b** (temp naming `Ch{N} · Character {i}`, characters track the chapters they appear in), and §9 **A20** (cast organized primary vs secondary). Roadmap: `design-docs/plans/book_view_redesign/01-roadmap.md` Workload 4.
