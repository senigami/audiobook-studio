...
- **Owner review round 2 (2026-06-12):** player bar = full-width bottom dock clarified (not part of the rail); Activity page confirmed as the global "what's going on" surface (in-flight + ETA + history + stats); Q6 refined — bar is HIDDEN when nothing is loaded (not grayed out; no false affordance). **All remaining north-star recommendations approved as written.** Queue drawer explicitly retained: check status from anywhere without losing your place (drawer = glance, Activity = depth).
- **Owner review round 3 (2026-06-12) — Studio/Review redefined (binding for Phase B):**
  - **Book view is the PRIMARY Studio mode** — prose paragraphs with speaker-colored sentence underlines, build status highlighting text in place, inline per-sentence play. "It's how people read books." **Script view demotes to a secondary preview mode** ("final read-through / play-script"), not the center of Studio. Amends Q3: still one ScriptView component, but the default routed mode is book view.
  - **Sub-sentence assignment is done IN book view** (the mixed «"He excelled," Dove said…» case) — quoted span carries the character's underline, remainder is narrator. Casting owns voice mapping; the span-level assignment gesture lives in the book-view text.
  - **Review = follow-along player**: text panel highlights the currently-playing sentence (past dimmed, auto-scroll, tap-to-seek), transport with replay/skip-back, annotations at timestamps, and re-render-section in place — the re-render highlight follows progress exactly like Studio's build view.
  - **Safe-text + section-number toggles survive** as dev-leaning view options; safe text is rendered per-engine and may differ per section depending on the voice's engine.
...
- **Owner review round 4 (2026-06-12) — space + ownership decisions (binding, mocked in v3.2):**
  ...
  - **The rail owns chapter navigation** (overriding the round-3 hold): full chapter list with status orbs + render bars + the ⋯ action menu nests under the Studio stage link; Studio's in-page chapter rail is REMOVED and the prose column takes the width. Hidden when the rail is collapsed. Owner confirmed round 5: chapters flooding the rail while on Studio is acceptable and better than the old dropdown selector — it's a quick-link list shown only when Studio is active.
- **Owner review round 5 (2026-06-12) — mocked in v3.3:**
  - **Top-bar book identity line**: inside a book, the breadcrumb area carries cover chip + title + "author · series · runtime · predicted" in one line; clicking it jumps to Publish (the canonical book-info editor). Replaces the Manuscript "Book details" disclosure (removed). Zero vertical cost — the bar height was already paid for.
  - **Manuscript owns the text — "author mode" is Manuscript done right (round 5b, mocked v3.4).** Per-chapter lifecycle `Draft → Ready → Cast → Rendered` (pill on every chapter row). Draft/Ready chapters are freely editable in Manuscript — no warnings (the paste-and-rewrite phase, and the author-writing-from-scratch case: a book whose chapters are all Drafts; "+ New chapter" opens a blank editor). Cast/Rendered chapters open read-only with an "Edit text" unlock gating the existing best-effort-assignment-matching warning — the one honest gate, only when there is something to lose. A "Focus" toggle gives the distraction-free full-width writing view (rail auto-collapses) — author mode is a presentation mode, NOT a new workflow or nav destination. Two edit scopes stay distinct: Manuscript = document-level (restructure/rewrite, accept assignment churn), Studio = surgical (fix a word, re-analyze affected sections only).
  - **StatusOrb is PRESERVED (owner, round 5c).** The existing chapter StatusOrb — status color PLUS the circumferential progress ring showing how much of the chapter is rendered — carries into the redesign wherever chapter status appears (rail chapter list, Manuscript table, Activity). The mock's plain colored dots are a low-fidelity stand-in only; the real build reuses the orb component. Do not regress to plain dots.
  - **Studio is the casting-assignment surface — voice PAINTING is the primary gesture**: a slim right-hand "Cast" palette (color dot + avatar + name per character, Narrator (default) first); click a swatch to arm the brush, click sentences in the prose to assign (re-underlines live). Palette placed in-page (next to the text) rather than in the rail: painting needs chapters-nav and palette simultaneously, and the rail is wayfinding. Sub-sentence spans extend the same gesture (planned). The Casting stage remains the roster/table view (who's in the book, voice per character); Studio is where per-line assignment happens.
