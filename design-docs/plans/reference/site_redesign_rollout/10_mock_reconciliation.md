...
| Plugin store and GitHub repo install | North star; R5 logs; owner decision | ZIP plugin import exists. Owner wants GitHub repo URL install for v2.0; open-ended GitHub topic browsing can defer if needed. | Mock now shows Upload ZIP, GitHub URL, trust analysis, and an official registry install path. | represented | Keep the mock interactive for end-state review. Implementation split: release = ZIP upload + paste GitHub repo URL install + official plugin registry + trust/dependency setup; post-v2 = open GitHub topic search/browse and richer update/pull UX. | P0 | Mock worker + platform worker | `99_progress_log.md` R5-T11; `StorePlaceholder.tsx`; `platform.tsx`; Owner decision 2026-06-15 |
| Official plugin registry previews | Owner decision | Owner is comfortable running an official registry. Registry entries can include summary, icon, tags, docs/homepage, repo URL, trust, compatibility; details can be fetched from repo files. | Mock now shows official registry entries, detail metadata, repository links, dependency preview, trust level, and install CTA. | represented | Keep registry browse/detail in the mock. Next implementation plan should define the registry JSON schema, hosting/source-of-truth, repo metadata enrichment, compatibility fields, trust labels, and installer handoff. Do not require arbitrary website scraping. | P0 | Mock worker + platform worker | Owner decision 2026-06-15; `frontend/src/demo/stages/siteMockup/panes/platform.tsx` |
...
| Specs conformance | Road to v2; docs/specs | SP9 conformance pass gates v2. Specs and code are jointly authoritative. | Mock can drift from specs if updated without final spec pass. | future-label | After mock update, run a spec-conformance checklist against current behavior and classify any remaining future labels. | P1 | Codex | `road_to_v2.md` Stage 6; `design-docs/specs/README.md` |

## Future Item Classification

The default classification below follows `design-docs/plans/final_release/road_to_v2.md` when a future item appears in multiple sources. If an explorer report disagreed with the release checklist, the release checklist wins until the owner changes it.
...
| Open GitHub plugin browse/search/update UX | post-v2 | Broad GitHub topic search/browse and richer update/pull UX can defer. Paste-a-GitHub-repo-URL install and owner-controlled official registry install remain release scope. |
...
`design-docs/plans/site_redesign_rollout/02_capability_inventory.md` reports 120/120 current capabilities re-homed. This reconciliation does not reopen those implementation checks; instead, every capability family is now either represented in the matrix as already covered, listed as `mock-missing`/`mock-stale`, or marked as intentionally omitted/dev-only/end-state prototype. After the first interactive mock update, the remaining high-risk follow-ups are: XTTS/Voxtral repo-download migration implementation detail, official registry schema/source-of-truth, API key/LAN/rate-limit security model, release/demo-bundle dependencies, and any owner-directed polish from private mock review.
...
```json
{
  ...
  "must_read_first": [
    ...
    "design-docs/plans/site_redesign_rollout/10_mock_reconciliation.md",
    "design-docs/plans/site_redesign_rollout/02_capability_inventory.md",
    "design-docs/plans/site_redesign_rollout/99_progress_log.md",
    "design-docs/plans/final_release/road_to_v2.md",
...
```json
{
  ...
  "read": [
    "design-docs/plans/site_redesign_rollout/02_capability_inventory.md",
    "design-docs/plans/site_redesign_rollout/99_progress_log.md",
    "design-docs/plans/final_release/04_voice_metadata_and_tagging.md",
    "design-docs/plans/final_release/road_to_v2.md",
...
```json
{
  ...
  "read": [
    "design-docs/plans/final_release/road_to_v2.md",
    "design-docs/plans/final_release/14_live_demo_revamp.md",
    "design-docs/plans/final_release/16_pinokio_distribution.md",
    "design-docs/plans/phases/phase_13_release_documentation_and_distribution.md",
    "design-docs/plans/master_agnostic_tasks.md",
    "docs/v1.html",
    "docs/demo/",
    "frontend/src/demo/"
