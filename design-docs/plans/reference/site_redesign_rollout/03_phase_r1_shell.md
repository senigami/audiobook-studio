...
- **Steps**:
  ...
  5. Bottom row per mock: expanded = one horizontal row with theme button (icon + "Dark mode"/"Light mode" label) filling the full width; collapsed = theme icon only. There is **no chevron in the bottom row** — the collapse affordance is the trailing-edge pill (see next point). Theme toggle flips `document.documentElement.dataset.theme` and calls `saveThemePref(next)` exactly like mock rail.tsx 52–57.
  6. Trailing-edge collapse handle: render a transparent `<button>` (full rail height, `width: 24px` for WCAG 2.5.8 hit target, `aria-expanded`, `aria-controls` pointing at the `<nav>`) containing a **thin vertical pill** span: `width: 14px`, `height: 44px`, `border-radius: 7px` (half-width = fully rounded sides), centered at 50% height via `align-items: center`. Pill background is transparent at rest; on hover, transitions to `var(--surface-alt)` and border-color to `var(--text-secondary)` (use CSS class `nav-rail__collapse-pill` with a `:hover` parent selector for CSS-based hover rather than JS). `aria-label="Collapse rail"`/`"Expand rail"`; clicking calls `setRailCollapsed(!collapsed)`. CSS class: `nav-rail__collapse-handle`. The pill design is chosen to double as a future drag-resize handle — do not make it circular or full-height.
...
- **Tests**: `NavRail.test.tsx` — renders all group labels and items; devMode localStorage flag adds Developer group; click Voices navigates (MemoryRouter); collapse-pill button click flips `studio-rail-collapsed` in localStorage and adds `nav-rail--collapsed` (button found by `aria-label="Collapse rail"`); badge shows for `queueCount=3` and hides for 0; theme button flips `data-theme` (assert both directions, satisfies R-E dark check); no second collapse button in the bottom row.
- **Verify**: standard three frontend commands.
