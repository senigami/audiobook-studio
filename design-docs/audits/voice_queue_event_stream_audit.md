...
> **Status (2026-06-11): Implemented.** `useQueueSync.ts`/`useJobs.ts` enforce queue.items-only row authority; allowed overlay fields live in `frontend/src/utils/queueOverlayFields.ts` (`QUEUE_OVERLAY_FIELDS`). Contract recorded in `design-docs/specs/live-events.md` §"Queue row authority" (1.2.0).
