...
> Adopt the approved **"Quiet Studio — Precision Pressroom"** visual language into the real app **and** the demo, as a token-layer re-skin of the existing CSS-variable system (no framework, no Tailwind), with `docs/specs/design-system.md` updated in lockstep.
...
2. `docs/specs/design-system.md` updated in lockstep (spec_version bumped, changelog rows, §2.4 contrast table recomputed against the new `--bg`); `voice-tone.md` updated only if copy changes (it shouldn't).
...
4. New/changed components (Switch, status icon-insets) follow TDD (`.agent/rules/verification.md`, `docs/specs/testing-standards.md`): failing test first, confirmed red, then implement.
...
- **Commit isolation:** a concurrent agent is consolidating `plans/`. Keep commits scoped to this work; this plan folder is its own isolated path.
