...
- Plans overlap: `plans/simplification/03_styling_separation.md` ST-3 lists these files as conversion targets. Sub-task A must align with, not conflict with, that plan. See "Overlap with plans/simplification/" below.
...
- Spec: `docs/specs/design-system.md` (rename documented)

## Target shape / contract

### The conversion rule (from `plans/simplification/03_styling_separation.md` ST-2/ST-3)
...
- After the rename, update `docs/specs/design-system.md` §2: remove `--accent` from the token registry; note it is retired.
...
## Overlap with `plans/simplification/`

`plans/simplification/03_styling_separation.md` ST-3 lists these same four files as conversion targets (items not in its top-15 by count, but they appear in the long tail). The alignment rule:
...
2. Sub-task A does NOT attempt the full ST-3 conversion of every inline style in these files. That is `plans/simplification/` territory.
3. When `plans/simplification/` ST-3 runs later, it should find these files already partially converted (no hardcoded hex, radii tokenized) and need only to extract the remaining static layout patterns.
...
7. Update `docs/specs/design-system.md` §6 entries for the four primitives: note radius + shadow tokens adopted; inline-style scope reduced. Bump spec_version (minor). Add changelog row.
...
- Do not convert the full ST-3 hotspot list from `plans/simplification/` — only the four primitives listed and only the hardcoded-color / pill-radius issues.
