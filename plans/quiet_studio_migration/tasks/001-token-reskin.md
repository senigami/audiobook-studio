...
| `docs/specs/design-system.md` | §2 token registry + §2.4 contrast table (R2) + §4 (if `--font-*` affects §4); bump `spec_version` to `1.6.0`; changelog row |
...
11. **Contrast recompute (R2)**: using the scratchpad contrast script pattern or a WCAG calculator, verify all pairs in the R2 table above. Record results in `docs/specs/design-system.md` §2.4. Confirm `--text-muted #8b95a8` on `--surface #1a1d27` (dark) passes AA (it should: ≈5.0:1).

12. **Update `docs/specs/design-system.md`**:
...
- `docs/specs/design-system.md` §2.1, §2.4, §8 updated as described in step 12.
