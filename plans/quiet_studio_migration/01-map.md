...
| `PART-spec` | Canonical spec | `docs/specs/design-system.md` (+ `voice-tone.md` iff copy) | Mirror every change in lockstep: §2/§2.4 (tokens + recomputed contrast vs new `--bg`), §4 (typeface), §6 (Switch, StatusOrb, progress, GlassInput), §8 (focus, reduced-motion), §10 (brand). |
...
- **INV-8 — Commit isolation.** Commits scoped to this work; coordinate with the concurrent `plans/` consolidation agent (different paths → no overlap, but rebase-aware).
