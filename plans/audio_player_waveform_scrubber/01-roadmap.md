...
- **005 — Scope-agnostic live player.** Implement `fitsLegibly(durationSec, barWidthPx)` and change `PlayerBar.tsx:121` from `forceWave ?? (scope === 'segment')` to `forceWave ?? fitsLegibly(...)`. **Remove the scope toggle**: delete `altScope`/`switchScope` from `playerBus.ts` (§2.1/§2.2), the scope-toggle UI + `player-scope-*` in `PlayerBar`/`components.css`, and stop adapters registering `altScope`. Make time `position/duration` (drop segment-relative). Vitest for representation choice + that no scope toggle renders.
