...
- [ ] **8. CI.** Add `build:demo` to the release checklist (doc 08) so the demo is rebuilt from the same commit as each release; stale demo = silent misrepresentation. **Note: `npm -C frontend run build:demo` must run at release.**
