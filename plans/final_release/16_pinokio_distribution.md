...
- [x] **PK1. Pin the Coqui fork.** `requirements-xtts.txt` line 2 installs `git+https://github.com/idiap/coqui-ai-TTS.git@main` — unreproducible and breaks if upstream moves. Pin to a specific tag or commit SHA. (This file is in THIS repo, not the wrapper — fix here, wrapper inherits.)
  _Done 2026-06-12: pinned to `ca2cf5155bca892ea820ad384400efbfac41b178` (main HEAD at time of pin). See `plugins/tts_xtts/requirements.txt`._
  _Acceptance: `requirements-xtts.txt` references an exact tag/SHA; fresh `--setup-only` run installs successfully._
- [x] **PK2. Torch backend selection.** `requirements-xtts.txt` has bare `torch>=2.10.0` — no CUDA/MPS/CPU differentiation. Windows/Linux users with NVIDIA GPUs may get CPU-only wheels (or the wrong CUDA), Apple Silicon gets MPS only if the default wheel supports it. Add platform detection in `run.sh`/`run.ps1` setup: detect `nvidia-smi` → install the matching CUDA index URL (`--index-url https://download.pytorch.org/whl/cu1xx`); Darwin arm64 → default wheel (MPS included); else CPU wheel. Log the chosen backend.
  _Done 2026-06-12: `select_torch_backend` (bash) / `Select-TorchBackend` (PS1) added to both scripts. nvidia-smi → CUDA cu128 index; Darwin arm64 → default PyPI (MPS); else CPU index. Override via `TORCH_BACKEND=cuda|mps|cpu` env var. CUDA index URL: `https://download.pytorch.org/whl/cu128` — Windows testers with older CUDA should set `TORCH_BACKEND=cuda` and adjust the index URL in the requirements install if cu128 is wrong for their driver._
...
- [ ] **PK4. `condarc` machine path is NOT shippable state — verify it's Pinokio-generated.** `/Users/stevendunn/pinokio/condarc` lines 8–9 hardcode `/Users/stevendunn/pinokio/bin/miniconda/...`. This file sits at the Pinokio *root* (generated per-machine by Pinokio itself), not inside the wrapper repo — confirm nothing from the wrapper repo references it; if the wrapper repo contains its own copy, delete it.
  _Audit 2026-06-12: `grep -rn "/Users/" ~/pinokio/api/audiobook-studio.pinokio.git/` (excluding `.git/` and `app/`) returned no matches. The wrapper repo contains zero absolute machine paths. PK4 acceptance criterion met — no changes needed in the wrapper repo._
