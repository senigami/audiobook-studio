...
| Text and progress utilities | Partial | Chunk grouping now uses engine metadata and redundant `app/utils/text_processing.py` is removed. Remaining generic text defaults and watchdog progress parsing still need separate classification before this area closes. | textops modules, progress parsing, plugin adapters | textops, progress, plugin tests |
| Storage and output routes | Partial | Root `engine_tests` is no longer a trusted storage root, voice readiness/bundle export use plugin manifest `test_sample` metadata, and broad `/projects` plus `/out/voices` static exposure has been replaced with explicit public asset routes. Remaining `xtts_audio` / `audio_out` migration references and output naming need classification before this area closes. | `app/config.py`, `app/api/utils.py`, asset routes, frontend API callers | API asset tests, frontend API tests |
