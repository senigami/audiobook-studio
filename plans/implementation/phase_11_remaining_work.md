...
| Storage and output routes | Partial | Root `engine_tests` is no longer a trusted storage root, and voice readiness/bundle export now use plugin manifest `test_sample` metadata instead of app-level sample filename assumptions. Remaining project/chapter asset routes and output naming still need classification before this area closes. | `app/config.py`, `app/api/utils.py`, asset routes, frontend API callers | API asset tests, frontend API tests |
