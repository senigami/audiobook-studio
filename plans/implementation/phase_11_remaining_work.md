...
| Failed queue entries do not show attempt date/time | Job and queue decommissioning / Frontend engine-agnostic cleanup | Open | Failed queue/job events include `updated_at` and `finished_at`, but the UI list of failed jobs does not show when the attempt happened. Expected: failed rows display a human-readable attempted/finished timestamp using existing job event fields. TDD first: add focused frontend queue/render-list test for failed item timestamp rendering. |
