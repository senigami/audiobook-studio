...
| Library home `/api/projects` load is slow | Storage and output routes / Final reference audit | Open | User observed `GET /api/projects` taking about 24 seconds. TDD first: add a timing/regression test around project listing that catches expensive per-project filesystem or manifest scans. |
