...
interface VoxtralSettingsProps {
...
const VoxtralSettings: React.FC<VoxtralSettingsProps> = ({
    mistralApiKey,
    setMistralApiKey,
    voxtralModel,
    setVoxtralModel,
...
const RefreshSection: React.FC<{
    onRefresh: () => void;
    hovered: boolean;
    onMouseEnter: () => void;
    onMouseLeave: () => void;
}> = ({ onRefresh, hovered, onMouseEnter, onMouseLeave }) => (
    <div 
    ...
    >
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <RefreshCw size={18} color={hovered ? 'var(--accent)' : 'var(--text-muted)'} />
            <div>
...
}> = ({ settings, onToggle, onRefresh, onShowNotification, hoveredItem, setHoveredItem }) => {
    return (
        <>
            {/* Safe Mode */}
            <div 
                style={rowStyle(hoveredItem === 'safe-mode')}
                onMouseEnter={() => setHoveredItem('safe-mode')}
                onMouseLeave={() => setHoveredItem(null)}
                onClick={() => onToggle('safe_mode', !!settings?.safe_mode)}
            >
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <button 
                    onClick={(e) => { e.stopPropagation(); onToggle('safe_mode', !!settings?.safe_mode); }}
                    className={settings?.safe_mode ? 'btn-primary' : 'btn-glass'} 
                    style={{ fontSize: '0.65rem', padding: '4px 10px', borderRadius: '6px', minWidth: '42px', cursor: 'pointer', border: 'none' }}
                >
            ...
            >
                <div 
                    style={rowStyle(hoveredItem === 'make-mp3')}
                    onMouseEnter={() => setHoveredItem('make-mp3')}
                    onClick={() => onToggle('make_mp3', !!settings?.make_mp3)}
                >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <button 
                        onClick={(e) => { e.stopPropagation(); onToggle('make_mp3', !!settings?.make_mp3); }}
                ...
                {settings?.make_mp3 && (
                    <div 
                        style={{ ...rowStyle(hoveredItem === 'sync'), paddingLeft: '28px' }}
                    ...
                    >
                        ...
                        <button 
                            ...
                            className="btn-glass"
                            style={{ fontSize: '0.65rem', padding: '4px 10px', borderRadius: '6px', minWidth: '42px', fontWeight: 700, color: 'var(--accent)', cursor: 'pointer', border: 'none' }}
                        >
                            START
                        </button>
                    </div>
