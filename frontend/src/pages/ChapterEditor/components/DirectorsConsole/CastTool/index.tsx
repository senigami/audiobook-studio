...
import { AlignLeft, AlertTriangle, BookOpen, Brush, Eye, Hash, Mic2 } from 'lucide-react';
...
/**
 * Cast mode — voice/speaker paint assignment. Ported from
 * `frontend/src/pages/Book/stages/StudioStage.tsx` (see
 * design-docs/plans/active/directors_console_activation/tasks/003-cast-tool.md).
 *
 * This resolves chapter/book context internally (INV-1, no props from
 * DirectorsConsole) exactly like StudioStage did.
 *
 * Deliberately deferred (see 00-overview.md): brush-size selection,
 * variation toggle, Match Voice, Stage Direction, Performance Cue,
 * mutation-batching.
 */
const CastToolBody: React.FC = () => {
  const {
    bookId,
    chapters,
    jobs,
  ...
  // Dirty-exit guard (DirtyGuardContext.tsx): Cast's own toolbar already
  // saves unsaved title/text edits before its "Save & prev/next" navigation
  // (navigateToChapter below); registering the same signal here protects
  // DirectorsConsole's own rail-tab switching too. NOTE: this does NOT cover
  // ChapterWorkspaceHeader's separate prev/next chapter-nav arrows, which
  // navigate without saving — that is a known, separate gap (see report).
  const { setDirty } = useDirtyGuard();
  useEffect(() => {
    setDirty(hasUnsavedChanges, hasUnsavedChanges ? 'Unsaved chapter text edit' : undefined);
        ...
        <div style={{ flex: 1, minWidth: 0, position: 'relative' }}>
          {selectedCharacterId && selectedCharacterId !== 'CLEAR_ASSIGNMENT' && (
            <div style={{
              ...
              Selected: {characters.find((char) => char.id === selectedCharacterId)?.name || 'Character'} — click sentences to assign
            </div>
          )}
...
/**
 * Cast mode — voice assignment (character swatch selection, span
 * click/drag assign, cast sidebar, analysis strip, render controls).
 * Ported from `StudioStage.tsx`; see
 * design-docs/plans/active/directors_console_activation/tasks/003-cast-tool.md.
 */
