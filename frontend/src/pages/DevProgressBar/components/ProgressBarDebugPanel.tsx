    ...
    <>
      ...
      <div style={{ marginTop: '1rem' }}>
        ...
        <MetricGrid items={[
          ...
          ['confidenceW', 'Auto-computed ETA trust weight w (doc 15). Ramps up with progress and ETA stability.', debugSnapshot?.etaConfidenceW != null ? debugSnapshot.etaConfidenceW.toFixed(3) : 'n/a'],
          ['confidenceBase', 'Adaptive base trust from ETA sample stability (cv-derived). Starts at BASE_FLOOR=0.2.', debugSnapshot?.etaConfidenceBase != null ? debugSnapshot.etaConfidenceBase.toFixed(3) : 'n/a'],
          ['confidenceCv', 'Coefficient of variation of recent end-time samples (lower=more stable).', debugSnapshot?.etaConfidenceCv != null ? debugSnapshot.etaConfidenceCv.toFixed(3) : 'n/a'],
          ['etaEndSmoothed', 'EMA-smoothed end-time estimate used for blending (ms since epoch).', debugSnapshot?.etaEndSmoothed != null ? new Date(debugSnapshot.etaEndSmoothed).toLocaleTimeString() : 'n/a'],
          ['slopeCappedVsRaw', 'Difference between clamped and blended end-time (ms). Positive=slowed, negative=sped up.', debugSnapshot?.slopeCappedVsRaw != null ? `${Math.round(debugSnapshot.slopeCappedVsRaw)}ms` : 'n/a'],
