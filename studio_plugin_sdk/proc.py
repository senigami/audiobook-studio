"""Subprocess streaming mechanism for engine plugins.

Stdlib-only: no ``app.*`` imports. Host policy (scratch dirs, config values)
is injected by the app-side wrappers in ``app/engines/proc_utils.py``.
"""
...
def coerce_subprocess_output(value) -> str:
    """Coerce raw subprocess output (bytes/str/other) to a safe str."""
