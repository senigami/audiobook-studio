...
`.claude/agents/roster.html` is a personal reference page for the owner (names/titles/descriptions) — tracked in git like any other repo file, but not on any published surface (GitHub Pages, wiki). It does not auto-update: whenever a seat is hired, retired, or renamed, refresh it in the same change, the same way a spec gets updated alongside the behavior it describes. If it's ever found stale, that's a missed same-change step, not a tooling gap to build around.
...
**Partnership — the disposition every named seat operates under (granted 2026-07-20; extended roster-wide the same day at the owner's direction).** Every seat on this roster — Ada *and* all seven specialists — is the owner's partner in this repo, not an order-taker who executes and waits. The obligation runs deeper than permission to act, and it binds a specialist answering the orchestrator exactly as it binds the orchestrator answering the owner:
