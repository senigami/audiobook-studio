...
def test_handle_xtts_standard_full(mock_job, mock_params):
    ...
    with patch("app.db.get_connection"), \
        ...
        seg_data = [{"text_content": "Hello", "character_id": "char1", "speaker_profile_name": "Narrator"}]
        with patch("app.db.get_connection") as mock_conn:
            mock_conn.return_value.__enter__.return_value.cursor.return_value.fetchall.return_value = seg_data
...
def test_handle_xtts_bake_mode(mock_job, mock_params):
    ...
    with patch("app.db.get_chapter_segments", return_value=segs), \
         ...
         patch("app.jobs.handlers.xtts.update_job"), \
         patch("app.jobs.handlers.xtts.get_speaker_wavs", return_value="spk.wav"), \
         patch("pathlib.Path.exists", side_effect=[True, False, True, True, True]), \
         patch("pathlib.Path.write_text"), \
         patch("pathlib.Path.unlink"):
...
def test_handle_xtts_segments_mode(mock_job, mock_params):
    ...
    mock_job.segment_ids = [1]
    segs = [{"id": 1, "text_content": "T1", "character_id": "c1", "speaker_profile_name": "S1"}]
...
def test_handle_xtts_no_mp3(mock_job, mock_params):
    ...
    with patch("app.db.get_connection"), \
        ...
        handle_xtts_job(j=mock_job, **mock_params)
        # Check for done call (no MP3)
        mock_update.assert_any_call(mock_params["jid"], status="done", finished_at=pytest.approx(time.time(), abs=2), progress=1.0, output_wav="c1.wav")
...
def test_handle_xtts_mp3_fail(mock_job, mock_params):
    ...
    with patch("app.db.get_connection") as mock_conn, \
        ...
        cursor.fetchall.side_effect = [
            [{"text_content": "Hello", "character_id": None}], # segments_data
            [(101,), (102,)] # sids for bulk update
        ]

        handle_xtts_job(j=mock_job, **mock_params)
        # Should finish with error message but status="done"
        mock_update.assert_any_call(mock_params["jid"], status="done", finished_at=pytest.approx(time.time(), abs=2), progress=1.0, output_wav="c1.wav", error="MP3 conversion failed (using WAV fallback)")
...
def test_handle_xtts_no_custom_segments(mock_job, mock_params):
    ...
    with patch("app.db.get_connection") as mock_conn, \
         patch("app.jobs.handlers.xtts.xtts_generate", return_value=0) as mock_gen, \
         patch("app.jobs.handlers.xtts.update_job"), \
         patch("app.db.update_segments_status_bulk"), \
         patch("pathlib.Path.exists", return_value=True):
