"""Plugin-local conftest so the Voxtral test suite can run standalone.
...
no-op — in-tree behavior is unchanged. Standalone (``cd tts_engines/tts_voxtral
...
_temp_dir = tempfile.TemporaryDirectory(prefix="voxtral-plugin-tests-")
