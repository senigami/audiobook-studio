...
class TestVoxtralSegments:
    ...
    def test_segment_job_renders_only_targeted_groups(self, tmp_path):
        ...
        with patch("tts_engines.tts_voxtral.plugin.studio.segments.generate_via_bridge", side_effect=fake_bridge), \
             patch("app.db.get_chapter_segments", return_value=segs), \
             patch("app.db.update_segment") as mock_update_seg, \
             patch("tts_engines.tts_voxtral.plugin.studio.segments._get_ctx") as mock_ctx_factory, \
             patch("tts_engines.tts_voxtral.plugin.studio.segments._handler") as mock_handler_factory:
...
class TestVoxtralBake:
    ...
    def test_bake_renders_only_missing_groups(self, tmp_path):
        ...
        with patch("tts_engines.tts_voxtral.plugin.studio.bake.generate_via_bridge", side_effect=fake_bridge), \
             ...
             patch("tts_engines.tts_voxtral.plugin.studio.bake._get_ctx") as mock_ctx_factory, \
             patch("tts_engines.tts_voxtral.plugin.studio.bake._handler") as mock_handler_factory:
...
class TestHandlerRouting:
    ...
    def test_handler_routes_segment_ids_job_to_segment_handler(self, tmp_path):
        ...
        with patch("tts_engines.tts_voxtral.plugin.studio.handler.get_chapter_dir", return_value=tmp_path), \
             patch("tts_engines.tts_voxtral.plugin.studio.handler.get_speaker_settings", return_value={}), \
             patch("tts_engines.tts_voxtral.plugin.studio.handler._chapter_uses_multiple_profiles", return_value=False), \
             patch("tts_engines.tts_voxtral.plugin.studio.handler.handle_voxtral_segments", return_value=0) as mock_seg, \
             patch("tts_engines.tts_voxtral.plugin.studio.handler.handle_voxtral_bake") as mock_bake, \
             patch("tts_engines.tts_voxtral.plugin.studio.handler.update_job"), \
             patch("tts_engines.tts_voxtral.plugin.studio.handler._get_ctx") as mock_ctx_f:
    ...
    def test_handler_routes_bake_job_to_bake_handler(self, tmp_path):
        ...
        with patch("tts_engines.tts_voxtral.plugin.studio.handler.get_chapter_dir", return_value=tmp_path), \
             patch("tts_engines.tts_voxtral.plugin.studio.handler.get_speaker_settings", return_value={}), \
             patch("tts_engines.tts_voxtral.plugin.studio.handler._chapter_uses_multiple_profiles", return_value=False), \
             patch("tts_engines.tts_voxtral.plugin.studio.handler.handle_voxtral_segments") as mock_seg, \
             patch("tts_engines.tts_voxtral.plugin.studio.handler.handle_voxtral_bake", return_value=0) as mock_bake, \
             patch("tts_engines.tts_voxtral.plugin.studio.handler.update_job"), \
             patch("tts_engines.tts_voxtral.plugin.studio.handler._get_ctx") as mock_ctx_f:
...
def _run_voxtral_bake_with_straggler_save(tmp_path, cancelled):
    ...
    with patch("tts_engines.tts_voxtral.plugin.studio.bake._get_ctx", return_value=mock_ctx), \
         patch("tts_engines.tts_voxtral.plugin.studio.bake.get_chapter_segments", return_value=segs), \
         patch("tts_engines.tts_voxtral.plugin.studio.bake.update_segment", side_effect=capture_update_segment), \
         patch("tts_engines.tts_voxtral.plugin.studio.bake.generate_via_bridge", side_effect=generate_side_effect), \
         patch("tts_engines.tts_voxtral.plugin.studio.bake._handler") as mock_handler_factory:
...
def _run_voxtral_segments_with_straggler_save(tmp_path, cancelled):
    ...
    with patch("tts_engines.tts_voxtral.plugin.studio.segments._get_ctx", return_value=mock_ctx), \
         patch("tts_engines.tts_voxtral.plugin.studio.segments.get_chapter_segments", return_value=segs), \
         patch("tts_engines.tts_voxtral.plugin.studio.segments.update_segment", side_effect=capture_update_segment), \
         patch("tts_engines.tts_voxtral.plugin.studio.segments.generate_via_bridge", side_effect=generate_side_effect), \
         patch("tts_engines.tts_voxtral.plugin.studio.segments._handler") as mock_handler_factory:
