...
def list_speaker_profiles():
    ...
    for name, d in sorted_items:
        ...
        if len([b for b in built_samples if SAFE_SAMPLE_NAME_RE.fullmatch(b) and find_secure_file(d, b)]) < len(built_samples):
...
def _rename_profile_folders(old_name: str, new_name: str):
    ...
    if old_dir and not new_dir.exists():
        if " - " in old_name and " - " in new_name and old_dir.parent == VOICES_DIR:
            ...
            new_dir = secure_join_flat(VOICES_DIR, new_name)
