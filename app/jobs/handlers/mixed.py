...
def _load_segments(chapter_id: str):
    ...
    with get_connection() as conn:
        ...
        cursor.execute(
            """
            SELECT s.text_content, s.character_id, s.id, c.speaker_profile_name, s.audio_status, s.audio_file_path
...
def _segment_profile_name(segment: dict, default_profile: str | None) -> str | None:
    return segment.get("speaker_profile_name") or default_profile
...
def _render_xtts_segment(segment: dict, profile_name: str | None, out_wav: Path, safe_mode: bool, on_output, cancel_check) -> int:
    if not profile_name:
...
def _render_voxtral_segment(segment: dict, profile_name: str | None, out_wav: Path, on_output, cancel_check) -> int:
    if not profile_name:
...
def handle_mixed_job(jid, j, start, on_output, cancel_check, text=None):
    from ...db import get_chapter_segments, get_connection, update_segment, update_segments_status_bulk
    ...
    if j.segment_ids:
        ...
        target_segments = [segment for segment in all_segments if segment["id"] in target_ids]
    elif j.is_bake:
        target_segments = [
            segment
            for segment in all_segments
            if segment.get("audio_status") != "done"
            or not _segment_output_path(pdir, segment["id"]).exists()
        ]
    else:
        target_segments = all_segments

    renderable_segments = [segment for segment in target_segments if (segment.get("text_content") or "").strip()]
    total_segments = len(renderable_segments)

    for index, segment in enumerate(renderable_segments, start=1):
        if cancel_check():
            update_job(jid, status="cancelled", finished_at=time.time(), progress=1.0, error="Cancelled.")
            return "cancelled"

        segment_id = segment["id"]
        profile_name = _segment_profile_name(segment, j.speaker_profile)
        engine = resolve_profile_engine(profile_name, "xtts")
        seg_out = _segment_output_path(pdir, segment_id)
        ...
        try:
            if engine == "voxtral":
                rc = _render_voxtral_segment(segment, profile_name, seg_out, on_output, cancel_check)
            else:
                rc = _render_xtts_segment(segment, profile_name, seg_out, j.safe_mode, on_output, cancel_check)
        ...
        progress = (index / total_segments) * progress_limit if total_segments else progress_limit
    ...
    fresh_segments = get_chapter_segments(j.chapter_id)
    segment_paths = []
    for segment in fresh_segments:
        if segment.get("audio_status") == "done" and segment.get("audio_file_path"):
            spath = pdir / segment["audio_file_path"]
            if spath.exists():
