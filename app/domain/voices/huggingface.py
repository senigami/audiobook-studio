...
def export_hf_voice_bundle(
    *,
    ...
    ``engine_assets`` maps ``engine_id -> [(filename, bytes), ...]`` for
    precomputed model files (e.g. ``latent.pth``) written under
    ``assets/<engine_id>/`` in the bundle, so a downloader gets instant use
    without re-cloning from the sample audio. Optional — a voice with no
    resolvable engine asset still exports the base bundle with no ``assets/``
    entries at all, not an empty ``assets/`` directory.

    ``sample_engine_id``, when given, is the engine/model that generated
    ``sample_mp3_bytes`` — the sample and any ``engine_assets`` must come
    from the same variant (the owner's explicit requirement: never pair a
    sample with an asset generated by a different model). Appended as a note
    to the generated README so the Hub card states which model produced the
    sample, without modifying ``bundles.generate_readme_md`` itself.
    ...
    readme = generate_readme_md(manifest_for_export)
    if sample_engine_id:
        readme = f"{readme}\n## Sample\n\nGenerated using the **{sample_engine_id}** engine/model.\n"
