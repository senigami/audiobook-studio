...
| 1.6.4 | 2026-06-20 | **§2.2 compliance (QW-7).** Registered two on-color tokens — `--text-on-error` (#ffffff, fixed in both themes) and `--text-on-warning` (#1c1300, fixed in both themes) — and converted the last five hardcoded color literals in real-app components to tokens: `StatusOrb` `!`-on-error and triangle-on-warning, `LiveOutputTable` two on-accent button labels (→ `--text-on-accent`), and the `ColorSwatchPicker` highlight dot (→ `--surface-glass-half`). No rendered-value change in either theme (the new error/warning tokens encode the same literals that were inline); brings the components into §2.2 (no raw color literals). |
