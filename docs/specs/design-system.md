...
`PredictiveProgressBar` is also a shared primitive but is fully specified in `progress-presentation.md` — see that spec; do not re-document its contract here. Visual additions from P3: a 12px terminus icon at the leading edge of the fill (visible when fill > 8%): `Clock` preparing, `Loader2` running, `Check` done, `X` failed/cancelled. The fill div receives `.is-running` when `isLiveAnimatedStatus(presentationState)` is true, wiring `@keyframes calm-pulse` via `--pulse-duration` (INV-5). The fill background uses `var(--progress-preparing-fill)` (solid slate) for preparing and `var(--live-indicator)` (full-opacity action blue) for running — both from tokens, no hardcoded colors. Props, ETA math, lane logic, and invariants are unchanged.
