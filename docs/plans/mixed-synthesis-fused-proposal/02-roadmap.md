...
Update `docs/specs/live-events.md` (preparing window / ETA suspension), `progress-presentation.md` (segment preparing tier, no synthetic lane), `queue-jobs.md` (per-group phase vs monotonic status); bump versions + changelog rows in the same change that alters the behavior.
